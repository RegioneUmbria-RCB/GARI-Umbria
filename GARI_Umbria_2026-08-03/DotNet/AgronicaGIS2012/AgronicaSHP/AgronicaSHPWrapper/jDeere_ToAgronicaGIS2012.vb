...
Public Class jDeere_ToAgronicaGIS2012
    ...
    Public Function convertConListachiavi(configurazioneImportazione As ConfigurazioneImportazione, ByVal ListaChiaviBoundaries As String, ByVal objParametri_server As AgronicaCoreParametri) As String
        ...
        Dim loaderJDeerefile As New jDeereController
        Dim jDeereBoundaries As rispostaStandard(Of jDeereDataModel_GenericList(Of jDeereDataModel_Boundary)) =
            loaderJDeerefile.RiportoBoundaryGIASLeggiOggetti(configurazioneImportazione, ListaChiaviBoundaries, objParametri_server)

        'esco se non trovo dati
        If jDeereBoundaries.RispostaStringa.total = 0 Then
            Return ""
        End If
        ...
        If Debugger.IsAttached Then
            stopME = -1
        End If
        For Each forma As jDeereDataModel_Boundary In jDeereBoundaries.RispostaStringa.values
            ...
            ' VAnni: 28/1/2021: capire il multipol e come estrarre i poligoni (inner, outer ?)
            Dim poligono As jDeereDataModel_Polygon =
                forma.multipolygons.First
            ...
            Dim idJohnDeereBoudaryID = "idJohnDeereBoudaryID"
            Dim testo As String = idJohnDeereBoudaryID & "§ " & forma.id & "|" & nome & "§ " & forma.name & "|"
            ...
            Dim boundaryXYZ = jDeereController.ListaPuntiJDToAgronicaXYZ(poligono.rings.First.points)
        ...
        For Each elemento In listOFEntita
            ...
            Try
                ...
                Dim OUTPUT_GISEntitaCod As Integer
                AgronicaCoreDataProvider.ConnessioniTransazioni.ApriConnessione(True, _objParametri_Server)
                ScriviElementiGrafici.scrivi(elemento.ToString, OUTPUT_GISEntitaCod, _objParametri_Server)

                Dim ElemJD As String = AgronicaCoreUtility.AgroGISHelper.GetValueFromGeoDes("idJohnDeereBoudaryID", elemento.Attribute("text"))

                AssociaEntitaGiasConEntitaJD(ElemJD, objParametri_server, ScriviAssociazioneGISJD, SeqLeggi, OUTPUT_GISEntitaCod)
    ...
    Private Sub AssociaEntitaGiasConEntitaJD(
        ...
        '    SeqLeggi.Agronica_SequenzaTabelle_NuovoID("jDeereDataModel_EntitaGIAS", _objParametri_Server)

        'Lavez - 12/07/2024 - normalizzazione chiamate a stack counter
        Dim jDeereDataModel_EntitaGIAS_COD As Integer =
            SeqLeggi.NuovoId_Tabella("jDeereDataModel_EntitaGIAS", 0, AgronicaCoreDataProvider.CostantiPersonalizzate.UpperBoundTabelle_Per_SequenzaTabelle_Topcode, _objParametri_Server)


        ScriviAssociazioneGISJD.ScriviAssociazioneGIASJD(
            enum_TipoEntitaJohnDeere.Boundary,
    ...
    Public Function convert(configurazioneImportazione As ConfigurazioneImportazione, objParametriServer As AgronicaCoreParametri, objParametriUtenti As AgronicaCoreParametri) As String Implements xxx_toAgronicaGIS2012.convert
