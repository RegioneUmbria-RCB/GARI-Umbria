    ...
    public class GeoTransform
    {
		...
		bool IsTrivial
        {
            get
            {
                ...
                return Math.Abs(transform[0]) < epsilon && Math.Abs(transform[1] - 1) < epsilon &&
                    Math.Abs(transform[2]) < epsilon && Math.Abs(transform[3]) < epsilon &&
                    Math.Abs(transform[4]) < epsilon && Math.Abs(transform[5] - 1) < epsilon;
            }
        }
