...
async function docR() {
    ...
    if (QS_Avanzamento == 2) {
        $("#FiltriPraticaApprovatore").show();
        $("#FiltriImpresa").hide();
        $("#filtriDettagli1").show();
        await dettaglio_create();
        KendoDDL('ddlLivelloDettaglio').value(1);
        ddlDettaglio_Change();
        await tipoPratica_create();
        await approvatore_load();
        await allevamenti_load();
        await lavorazioni_load();
        await colture_load();
    ...
    $($("#tabstrip_elenco").data("kendoTabStrip").items()[2]).attr("style", "display:none");
    ...
    if (QS_Avanzamento === 1)
        tempStr = "ELENCO RENDICONTAZIONI";
    else if (QS_Avanzamento === 0)
        tempStr = "ELENCO RICHIESTE";
    else
        tempStr = "RICERCA PER MACROUSI E LAVORAZIONI";

    $(".elenco")[0].children[1].innerHTML = tempStr

    window.addEventListener('message', event => {
        if (verificaOriginSecondaria(window, window.origin, event) &&
            (typeof event.data == "string") && event.data.includes("RispostaStringa")) {
