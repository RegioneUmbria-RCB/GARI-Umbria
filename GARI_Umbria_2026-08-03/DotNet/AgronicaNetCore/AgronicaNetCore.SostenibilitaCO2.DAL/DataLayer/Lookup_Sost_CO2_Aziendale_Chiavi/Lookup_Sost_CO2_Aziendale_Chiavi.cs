...
{
    public class Lookup_Sost_CO2_Aziendale_Chiavi : BaseDALSostenibilitaCO2, ILookup_Sost_CO2_Aziendale_Chiavi
    {
        private readonly IAgro_Sequence _sequenceDal;

        public Lookup_Sost_CO2_Aziendale_Chiavi(IServiceProvider provider, IStringLocalizer<Messages> localizer, bool securityBypass = false)
            : base(provider, localizer, securityBypass)
        {
            _sequenceDal = provider.GetRequiredService<IAgro_Sequence>();
        }

        private static WriteLookupSostCO2AziendaleChiavi Valorizza(WriteLookupSostCO2AziendaleChiavi dto)
        {
            dto.Filiera ??= "";
            dto.Azienda ??= "";
        ...
        public async Task<DataTable> ReadAsync(GetSostCO2Aziendale dto, AgronicaCoreParametri objP, FiltroAggiuntivo? xFiltroAggiuntivo = null)
        {
            var stbQuery = new StringBuilder();
            var sqlParams = new Dictionary<string, object>();

            stbQuery.AppendLine("SELECT * FROM Lookup_Sost_CO2_Aziendale_Chiavi ")
            ...
            if (!string.IsNullOrEmpty(dto.Azienda))
            {
                sqlParams.TryAdd("@azienda", dto.Azienda);
                stbQuery.AppendLine("    AND cuaa_azienda = @azienda ");
            }
            if (!string.IsNullOrEmpty(dto.Filiera))
            {
                sqlParams.TryAdd("@filiera", dto.Filiera);
                stbQuery.AppendLine("    AND cuaa_filiera = @filiera ");
            }
            if (dto.Anno != 0)
            {
        ...
        public async Task<bool> ExistAsync(int id, AgronicaCoreParametri objP)
        {
            ...
            var stbQuery = new StringBuilder();
            stbQuery.AppendLine("SELECT TOP(1) id FROM Lookup_Sost_CO2_Aziendale_Chiavi ")
        ...
        public async Task<bool> CreateAsync(WriteLookupSostCO2AziendaleChiavi dto, AgronicaCoreParametri objP)
        {
            dto = Valorizza(dto);
            ...
            var stbQuery = new StringBuilder();
            stbQuery.AppendLine("INSERT INTO Lookup_Sost_CO2_Aziendale_Chiavi ")
                    ...
                    .AppendLine("VALUES ")
                    .AppendLine("    (@id, @idInv, @dataInv, @anno, @filiera, @azienda,")
            ...
            expandoObj.TryAdd("@filiera", dto.Filiera);
            expandoObj.TryAdd("@azienda", dto.Azienda);
        ...
        public async Task<bool> UpdateAsync(WriteLookupSostCO2AziendaleChiavi dto, AgronicaCoreParametri objP)
        {
            if (dto.Id <= 0)
                throw new Exception("id non valorizzato.");
            ...
            const string updateQuery =
                @"UPDATE Lookup_Sost_CO2_Aziendale_Chiavi SET
                    data_invocazione  = @dataInv,
                    anno              = @anno,
                    cuaa_filiera           = @filiera,
                    cuaa_azienda           = @azienda,
                    Inviato           = @inviato,
                    DataInvio         = @dataInvio,
        ...
        public async Task<bool> DeleteAsync(int id, string idInvocazione, AgronicaCoreParametri objP)
        {
            ...
            const string deleteQuery =
                "DELETE FROM Lookup_Sost_CO2_Aziendale_Chiavi WHERE id = @id AND id_invocazione = @idInv";
        ...
        public async Task<string> ScriviModificaAsync(WriteLookupSostCO2AziendaleChiavi dto, AgronicaCoreParametri objP)
        {
            if (string.IsNullOrWhiteSpace(dto.Id_Invocazione))
                throw new Exception("id_invocazione non valorizzato.");
            ...
            try
            {
                ...
                if (dto.Id <= 0)
                {
                    dto.Id = await _sequenceDal.NuovoId_TabellaAsync("lookup_sost_co2_aziendale_chiavi", 0, 2000000000, objP);
                    isNew = true;
                }
                else
        ...
        public async Task<bool> EliminaAsync(WriteLookupSostCO2AziendaleChiavi dto, AgronicaCoreParametriServer objP)
        {
            try
            {
        ...
        public async Task<bool> EliminaAsync(List<WriteLookupSostCO2AziendaleChiavi> dtos, AgronicaCoreParametriServer objP)
        {
