...
{
    public class Lookup_Sost_CO2_Aziendale_Payload : BaseDALSostenibilitaCO2, ILookup_Sost_CO2_Aziendale_Payload
    {
        public Lookup_Sost_CO2_Aziendale_Payload(IServiceProvider provider, IStringLocalizer<Messages> localizer, bool securityBypass = false)
            : base(provider, localizer, securityBypass)
        {
        }

        private static WriteLookupSostCO2AziendalePayload Valorizza(WriteLookupSostCO2AziendalePayload dto)
        {
            dto.Json_Richiesta ??= "";
            dto.Json_Risposta ??= "";
            ...
            if (dto.Data_Creazione == default)
                dto.Data_Creazione = DateTime.UtcNow;

            if (dto.Data_Modifica == default)
                dto.Data_Modifica = DateTime.UtcNow;
        ...
        public async Task<DataTable> ReadAsync(string? idInvocazione, short? inviato, AgronicaCoreParametri objP, FiltroAggiuntivo? xFiltroAggiuntivo = null)
        {
            ...
            stbQuery.AppendLine("SELECT * FROM Lookup_Sost_CO2_Aziendale_Payload ")
        ...
        public async Task<bool> ExistAsync(string idInvocazione, AgronicaCoreParametri objP)
        {
            ...
            var stbQuery = new StringBuilder();
            stbQuery.AppendLine("SELECT TOP(1) id_invocazione FROM Lookup_Sost_CO2_Aziendale_Payload ")
        ...
        public async Task<bool> CreateAsync(WriteLookupSostCO2AziendalePayload dto, AgronicaCoreParametri objP)
        {
            dto = Valorizza(dto);

            var stbQuery = new StringBuilder();
            stbQuery.AppendLine("INSERT INTO Lookup_Sost_CO2_Aziendale_Payload ")
        ...
        public async Task<bool> UpdateAsync(WriteLookupSostCO2AziendalePayload dto, AgronicaCoreParametri objP)
        {
            ...
            const string updateQuery =
                @"UPDATE Lookup_Sost_CO2_Aziendale_Payload SET
        ...
        public async Task<bool> DeleteAsync(string idInvocazione, AgronicaCoreParametri objP)
        {
            ...
            const string deleteQuery =
                "DELETE FROM Lookup_Sost_CO2_Aziendale_Payload WHERE id_invocazione = @idInv";
        ...
        public async Task<string> ScriviModificaAsync(WriteLookupSostCO2AziendalePayload dto, AgronicaCoreParametri objP)
        {
            if (string.IsNullOrWhiteSpace(dto.Id_Invocazione))
                throw new Exception("id_invocazione non valorizzato.");
        ...
        public async Task<bool> EliminaAsync(WriteLookupSostCO2AziendalePayload dto, AgronicaCoreParametriServer objP)
        {
            try
            {
        ...
        public async Task<bool> EliminaAsync(List<WriteLookupSostCO2AziendalePayload> dtos, AgronicaCoreParametriServer objP)
        {
