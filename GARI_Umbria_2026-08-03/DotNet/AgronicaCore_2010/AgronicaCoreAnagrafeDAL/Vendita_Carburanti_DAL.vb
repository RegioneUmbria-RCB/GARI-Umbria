...
Public Class Vendita_Carburanti_DAL
    ...
    Public Function OttienePIVADallePratiche(anno As Integer, cuaa As String) As String
        Dim imprese = _EFContext.Pratiche.Where(Function(s) s.Servizio_Cod = enum_Servizi.Gestione_UMA AndAlso s.Anno = anno AndAlso s.Cuaa = cuaa).FirstOrDefault()
        Return imprese.Piva
    End Function
    ...
    Public Function CaricaCarburantiVendita(filtri As FiltriCaricaRigheVenditaCarburanti,
        ...
        StrSQL.Append("SELECT Id_Vendite, Uma_Vendite.PivaSuperUser, PIVA_Venditore, PIVA_Cliente, Anno As Anno_Cod, Anno As Anno_Des, Conto_Proprio_Terzi As Conto_Proprio_Terzi_Cod,
                         ...
                         CASE Tipo_Documento
                           WHEN 1 THEN 'DAS'
                           WHEN 2 THEN 'Fattura accompagnatoria'
        ...
        If Filtro_Visibilita_Utente AndAlso Not filtri.NuovaVisibilita Then
            StrSQL.AppendLine(" LEFT JOIN Utenti_Visibilita_Appoggio uv_venditore ON Uma_Vendite.PIVA_Venditore = uv_venditore.Piva AND uv_venditore.Sa_Cod = 0 AND uv_venditore.Username = '" & _coreDBContext_user.UtenteUsername & "' ")
            StrSQL.AppendLine(" LEFT JOIN Utenti_Visibilita_Appoggio uv_cliente ON Uma_Vendite.PIVA_Cliente = uv_cliente.Piva AND uv_cliente.Sa_Cod = 0 AND uv_cliente.Username = '" & _coreDBContext_user.UtenteUsername & "' ")
        End If
        ...
        If filtri.Piva <> "-1" AndAlso filtri.Piva <> "" Then
            StrSQL.Append(" AND (UMA_Vendite.PIVA_Venditore = '" & filtri.Piva & "' OR UMA_Vendite.PIVA_Cliente = '" & filtri.Piva & "' ) ")
        End If
        ...
        If filtri.NuovaVisibilita And Not VisibilitaTotale Then
    ...
    Public Function OttieneLtAssegnati(filtri As FiltriAggiornaLtAssegnati) As Integer
        ...
        If filtri.ContoProprioTerzi = 0 Then
            ...
            StrProprioTerzi.Append(" GROUP BY Z.Piva, Z.Tipo_Carburante")

        Else
            StrProprioTerzi.AppendLine(" SELECT UMA_Richieste_Testata.piva, 2 As Tipo_Carburante,SUM(ISNULL(UMA_Richieste_Lavorazioni.Fabbisogno_Assegnato, 0)) +  ")
            StrProprioTerzi.AppendLine(" SUM(ISNULL(rsl.Approvazione_Iniziale_Gasolio, 0)) as Assegnato  ")
            StrProprioTerzi.AppendLine(" FROM UMA_Richieste_Testata  ")
            StrProprioTerzi.AppendLine(" JOIN Pratiche ON UMA_Richieste_Testata.Pratica_Cod = Pratiche.Pratica_Cod  ")
            StrProprioTerzi.AppendLine(" JOIN Pratiche_Stati_Attuali ON Pratiche_Stati_Attuali.Pratica_Cod = Pratiche.Pratica_Cod  ")
            StrProprioTerzi.AppendLine(" LEFT JOIN Richieste_Terzisti_Senza_Lavorazioni rsl ON rsl.richiesta_Cod = UMA_RIchieste_Testata.Richiesta_Cod ")
            ...
            StrProprioTerzi.AppendLine(" AND Pratiche.Anno = 2022  ")
            StrProprioTerzi.AppendLine(" AND UMA_Richieste_Testata.Piva = '{1}'  ")
            StrProprioTerzi.AppendLine(" AND UMA_Richieste_Testata.Tipo_Richiesta = {2}  ")
            StrProprioTerzi.AppendLine(" AND Pratiche_Stati_Attuali.Stato_Cod IN (2003, 2005, 2008)  ")
            StrProprioTerzi.AppendLine(" GROUP BY UMA_Richieste_Testata.piva  ")
            StrProprioTerzi.AppendLine(" UNION ALL  ")
            StrProprioTerzi.AppendLine(" SELECT UMA_Richieste_Testata.piva, 3 As Tipo_Carburante, SUM(ISNULL(UMA_Richieste_Lavorazioni.Fabbisogno_Assegnato, 0)) +   ")
            StrProprioTerzi.AppendLine(" SUM(ISNULL(rsl.Approvazione_Iniziale_Benzina, 0)) as Assegnato  ")
            StrProprioTerzi.AppendLine(" FROM UMA_Richieste_Testata  ")
            StrProprioTerzi.AppendLine(" JOIN Pratiche ON UMA_Richieste_Testata.Pratica_Cod = Pratiche.Pratica_Cod  ")
            StrProprioTerzi.AppendLine(" JOIN Pratiche_Stati_Attuali ON Pratiche_Stati_Attuali.Pratica_Cod = Pratiche.Pratica_Cod  ")
            StrProprioTerzi.AppendLine(" LEFT JOIN Richieste_Terzisti_Senza_Lavorazioni rsl ON rsl.richiesta_Cod = UMA_RIchieste_Testata.Richiesta_Cod ")
            ...
            StrProprioTerzi.AppendLine(" GROUP BY UMA_Richieste_Testata.piva  ")
            StrProprioTerzi.AppendLine(" UNION ALL  ")
            StrProprioTerzi.AppendLine(" SELECT UMA_Richieste_Testata.piva, 8 As Tipo_Carburante, SUM(ISNULL(UMA_Richieste_Lavorazioni.Fabbisogno_Assegnato, 0)) + ")
            StrProprioTerzi.AppendLine(" SUM(ISNULL(rsl.Approvazione_Iniziale_Gasolio_Serra, 0)) as Assegnato  ")
            StrProprioTerzi.AppendLine(" FROM UMA_Richieste_Testata  ")
            StrProprioTerzi.AppendLine(" JOIN Pratiche ON UMA_Richieste_Testata.Pratica_Cod = Pratiche.Pratica_Cod  ")
            StrProprioTerzi.AppendLine(" JOIN Pratiche_Stati_Attuali ON Pratiche_Stati_Attuali.Pratica_Cod = Pratiche.Pratica_Cod  ")
            StrProprioTerzi.AppendLine(" LEFT JOIN Richieste_Terzisti_Senza_Lavorazioni rsl ON rsl.richiesta_Cod = UMA_RIchieste_Testata.Richiesta_Cod ")
        ...
        StrSQL.AppendLine(" WHERE Avanzamento_Richiesta = 0")
        ...
        StrSQL.AppendLine(" ),")
        StrSQL.AppendLine(" Richieste_Terzisti_Senza_Lavorazioni(Richiesta_Cod, Approvazione_Iniziale_Gasolio, Approvazione_Iniziale_Benzina, Approvazione_Iniziale_Gasolio_Serra) As (")
        StrSQL.AppendLine(" SELECT t.Richiesta_Cod, t.Approvazione_Iniziale_Gasolio, t.Approvazione_Iniziale_Benzina, t.Approvazione_Iniziale_Gasolio_Serra")
        StrSQL.AppendLine(" FROM UMA_Richieste_Testata t")
        StrSQL.AppendLine(" where t.Carburante_Calcolato = 0 AND (Richiesta_Iniziale_Gasolio > 0 OR Richiesta_Iniziale_Benzina > 0 OR Richiesta_Iniziale_Gasolio_Serra > 0) ")
        StrSQL.AppendLine(" ),")
        StrSQL.AppendLine(" Totali_Assegnati(Piva, Tipo_Carburante, Assegnato) AS (")
        StrSQL.AppendLine(StrProprioTerzi.ToString())
        StrSQL.AppendLine(" )")
        StrSQL.AppendLine(" SELECT Totali_Assegnati.Piva, Totali_Assegnati.Tipo_Carburante, Totali_Assegnati.Assegnato - prima_richiesta_anno.rimanenza_gas As Assegnato")
        StrSQL.AppendLine(" FROM Totali_Assegnati")
        StrSQL.AppendLine(" INNER JOIN prima_richiesta_anno ON prima_richiesta_anno.piva = Totali_Assegnati.Piva")
        StrSQL.AppendLine(" where Totali_Assegnati.Tipo_Carburante = 2")
        StrSQL.AppendLine(" UNION ALL")
        StrSQL.AppendLine(" SELECT Totali_Assegnati.Piva, Totali_Assegnati.Tipo_Carburante, Totali_Assegnati.Assegnato - prima_richiesta_anno.rimanenza_benz As Assegnato")
        StrSQL.AppendLine(" FROM Totali_Assegnati")
        StrSQL.AppendLine(" INNER JOIN prima_richiesta_anno ON prima_richiesta_anno.piva = Totali_Assegnati.Piva")
        StrSQL.AppendLine(" where Totali_Assegnati.Tipo_Carburante = 3")
        StrSQL.AppendLine(" UNION ALL")
        StrSQL.AppendLine(" SELECT Totali_Assegnati.Piva, Totali_Assegnati.Tipo_Carburante, Totali_Assegnati.Assegnato - prima_richiesta_anno.rimanenza_ser As Assegnato")
        StrSQL.AppendLine(" FROM Totali_Assegnati")
        StrSQL.AppendLine(" INNER JOIN prima_richiesta_anno ON prima_richiesta_anno.piva = Totali_Assegnati.Piva")
    ...
    Public Function FiltriDettaglioLtAcquistabili(params As FiltriDettaglioLtAcquistabili) As DettaglioLtAcquistabiliRisultati
        'Dim risultato As New DettaglioLtAcquistabiliRisultati()

        'Dim strSQL As New Text.StringBuilder()
        'strSQL.Append("SELECT Anno, Tipo_Carburante, Conto_Proprio_Terzi, sum(lt) as TotaleLtAcquistati
        '               FROM UMA_Vendite
        '               Join Imprese_Codici ic on ic.PIVA = UMA_Vendite.PIVA_Cliente and ic.id_cod = 1010
        '               WHERE ic.val_cod = 'QNTRLD57C25C446X'
        '               GROUP BY UMA_Vendite.Anno, UMA_Vendite.tipo_Carburante, UMA_Vendite.Conto_Proprio_Terzi")

        'Dim dt = EseguiQuery_Lettura(_coreDBContext, strSQL.ToString(), MethodBase.GetCurrentMethod().Name)

        'Dim ltAcquistati As New Dictionary(Of String, Double)
        'For Each dr As DataRow In dt.Rows
        '    If dr.Item("Anno") = params.Anno AndAlso dr.Item("Tipo_Carburante") = params.TipoCarburante Then
        '        If dr.Item("Conto_Proprio_Terzi") = 0 Then
        '            ltAcquistati.Add("ContoProprio", dr.Item("TotaleLtAcquistati"))
        '        Else
        '            ltAcquistati.Add("ContoTerzi", dr.Item("TotaleLtAcquistati"))
        '        End If
        '    End If
        'Next
    ...
    Public Function OttieneTotaleLtAcquistati(parametri As FiltriAggiornaTotaleLtAcquistati) As Integer
        ...
        StrSQL.Append("SELECT Anno, Tipo_Carburante, sum(lt) as TotaleLtAcquistati
                       FROM UMA_Vendite
                       Join Imprese_Codici ic on ic.PIVA = UMA_Vendite.PIVA_Cliente and ic.id_cod = 1010
                       WHERE ic.val_cod = '" & parametri.Cuaa & "'
                       AND UMA_Vendite.Conto_Proprio_Terzi = " & parametri.ContoProprioContoTerzi & "
