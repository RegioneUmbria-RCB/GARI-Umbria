...
Public Class Vendita_Carburanti
    ...
    Private Function TentativoDettaglioLtAcquistabili(params As FiltriDettaglioLtAcquistabili) As RispostaStandard
        Dim totaleAcquistatiProprio = _carburantiDAL.OttieneTotaleLtAcquistati(New FiltriAggiornaTotaleLtAcquistati(params.Anno, params.Cuaa, params.Tipo_Carburante, ContoProprioTerzi.ContoProprio))
        ...
        Dim ltAcquistabiliProprio = ltAssegnatiProprio - totaleAcquistatiProprio

        If ltAssegnatiProprio = 0 AndAlso totaleAcquistatiProprio < 0 Then
            ltAcquistabiliProprio = 0
        End If

        Dim totaleAcquistatiTerzi = _carburantiDAL.OttieneTotaleLtAcquistati(New FiltriAggiornaTotaleLtAcquistati(params.Anno, params.Cuaa, params.Tipo_Carburante, ContoProprioTerzi.ContoTerzi))
        ...
        Dim ltAcquistabiliTerzi = ltAssegnatiTerzi - totaleAcquistatiTerzi

        If ltAssegnatiTerzi = 0 AndAlso totaleAcquistatiTerzi < 0 Then
            ltAcquistabiliTerzi = 0
        End If
    ...
    Private Sub ControllaValiditaRighe(carburanti As List(Of VenditaCarburantiDto))
        ...
        For Each carburante In carburanti
            ...
            errori += CarburantiHelper.ControllaPresenzaRendicontazione(carburante, _efContext)
        Next
        If Not String.IsNullOrEmpty(errori) Then
            Throw New Exception(errori)
        End If
    ...
    Public Function CaricaTipoDocumentiDDL() As List(Of Tipo_Documento_DDL)
        ...
        tipiDocumento.Add(New Tipo_Documento_DDL(1, "E-DAS"))
        tipiDocumento.Add(New Tipo_Documento_DDL(2, "Doc. cartaceo (causa di forza maggiore)"))
...
Public Class CarburantiHelper
    ...
    Public Shared Function ControllaPresenzaCampiObbligatori(carburante As VenditaCarburantiDto)
        ...
        Dim dataCreazione = If(carburante.Data_Creazione.HasValue, carburante.Data_Creazione.Value.Date, Today)
        If carburante.Data_Documento.HasValue AndAlso carburante.Data_Documento.Value < dataCreazione Then
            If IsNothing(carburante.Note_Rivenditore) OrElse String.IsNullOrEmpty(carburante.Note_Rivenditore) OrElse Trim(carburante.Note_Rivenditore) = "" Then
                stringaErrore &= "E’ obbligatorio registrare il documento nello stesso giorno dell’emissione, questo per regolarizzare l’acquisto da parte dell’azienda. In casi di ritardo è obbligatorio indicarne il motivo nel campo Note e l’inadempienza sarà comunque segnalata all’Agenzia delle Dogane e Guardia di Finanza. <br/>"
            End If
        End If
    ...
    Friend Shared Function ControllaPresenzaRendicontazione(carb As VenditaCarburantiDto, efC As Gias_DeveloperServer_Entities) As String
        Dim exists As Boolean = False
        Dim comp As Integer
        Dim bocciate As Integer
        Dim altriStati As Integer
        Dim temp As Boolean
        Dim pratiche As IQueryable(Of Pratiche)
        Dim pratica As Pratiche
        Dim rend = efC.UMA_Richieste_Testata.Where(Function(s) s.Piva = carb.PIVA_Cliente AndAlso s.Tipo_Richiesta = carb.Conto_Proprio_Terzi_Cod AndAlso s.Avanzamento_Richiesta = 1)
        For Each rendicontazione In rend

            pratiche = efC.Pratiche.Where(Function(p) p.Pratica_Cod = rendicontazione.Pratica_Cod AndAlso p.Anno = carb.Anno_Cod)

            If pratiche.Count > 0 Then

                pratica = pratiche.First

                temp = efC.Pratiche_Stati_Attuali.Any(Function(p) p.Pratica_Cod = pratica.Pratica_Cod AndAlso p.Stato_Cod > TipiEnumerativi.enum_WAnagraficaStati.In_Compilazione)
                If temp Then
                    If efC.Pratiche_Stati_Attuali.Any(Function(p) p.Pratica_Cod = pratica.Pratica_Cod AndAlso p.Stato_Cod = TipiEnumerativi.enum_WAnagraficaStati.Verifica_Intermedia_Non_Superata) Then
                        bocciate += 1
                    Else
                        altriStati += 1
                    End If
                Else
                    comp += 1
                End If
                exists = exists OrElse temp

            End If
        Next
        If exists AndAlso Not (bocciate > 0 AndAlso comp = 1 AndAlso altriStati = 0) Then
            Return "E' presente una rendicontazione per questa azienda per l'anno specificato, non è possibile proseguire <br/>"
        End If
    ...
    Public Function OttieneTipoDocuemnto(cod As Integer)
        Select Case cod
            Case 1
                Return "E-DAS"
            Case 2
                Return "Doc. cartaceo (causa di forza maggiore)"
            Case Else
