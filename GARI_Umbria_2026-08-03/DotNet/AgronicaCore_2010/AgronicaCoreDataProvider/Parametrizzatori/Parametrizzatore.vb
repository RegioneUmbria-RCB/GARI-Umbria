...
Public Class Parametrizzatore
    ...
    Public Function Parametrizza(ByRef stringaSql As String,
        ...
        Try
            ...
            If Not checkOK Then
                ...
                If Not IsNothing(parametriOutput) AndAlso parametriOutput.Any Then
                    ...
                    Next
                End If
                Dim messaggioErrore = String.Format("---> MANCATA PARAMETRIZZAZIONE DELLA QUERY : {0}" & vbCrLf & "FALLITO DOPPIO CHECK DI SICUREZZA" & vbCrLf & "METODO CHIAMANTE: {1}",
            ...
            If checkOK Then
                ...
                If Not orderByOk Then
                    ...
                    Dim messaggioErrore = String.Format("---> MANCATA PARAMETRIZZAZIONE DELLA QUERY : {0}" & vbCrLf & "FALLITA PARAMETRIZZAZIONE ORDER BY" & vbCrLf & "METODO CHIAMANTE: {1}",
                                                       sbLog.ToString(), DammiStackTrace())
        ...
        Catch ex As OutOfMemoryException
            ...
            Dim messaggioErrore = String.Format("---> RILEVATA STRINGA SQL ECCESSIVAMENTE LUNGA PER IL PARSER : {0}" & vbCrLf & "METODO CHIAMANTE: {1}",
        ...
        Catch ex As TempoParserException
            ...
            Dim messaggioErrore = String.Format("---> IL PARSER SQL HA IMPIEGATO PIU' DI {0} MILLISECONDI DI TEMPO PER VALUTARE LA STRINGA. {1} " & vbCrLf & "METODO CHIAMANTE: {2}",
    ...
    Private Function ParametrizzaOrderBY(ByRef stringaSql As String, stringSqlOrigin As String,
        ...
        Try
            ...
            If overTime Then
                Dim messaggioWarning = String.Format("---> RILEVATA STRINGA SQL CON TEMPO PARSER ELEVATO: ( {0} millisecondi )" & vbCrLf & "{1}" & vbCrLf & "METODO CHIAMANTE: {2}", tempoParser,
                                                       stringaSqlOriginale, DammiStackTrace())

                Logga_Solo_DB(nomeRoutine, " *** WARNING PARAMETRIZZAZIONE NEL DATAPROVIDER ", messaggioWarning, objParametri_Server)
                Throw New TempoParserException(messaggioWarning)
            End If

            Dim xmlDoc As New XmlDocument()
            xmlDoc.LoadXml(stringaXml)

            Dim orderByNodes As XmlNodeList = xmlDoc.SelectNodes(".//SqlOrderByClause")

            If Not IsNothing(orderByNodes) AndAlso orderByNodes.Count > 0 Then

                Dim progressivoParametriOrderBy = parametriInput.First.Key + 1

                For cont As Integer = (orderByNodes.Count - 1) To -1 Step -1

                    Dim orderByNode As XmlNode = orderByNodes.Item(cont)
                    If Not IsNothing(orderByNode) Then

                        Dim orderBySB As New StringBuilder
                        Dim orderByBody As String = orderByNode.FirstChild.InnerText()
                        ...
                        Else
                            ...
                            Dim elencoCampi = Ottieni_Lista_Campi_Select(orderByNode).Distinct().ToList()

                            ' Ricavo la lista dei campi in order by
                            Dim orderByFields As XmlNodeList = orderByNode.SelectNodes(".//SqlOrderByItem")
                            For Each orderByField As XmlNode In orderByFields
                                ...
                                Dim originalCompleteFieldName As String = orderByField.ChildNodes.Item(1).FirstChild.InnerText
                                Dim completeFieldName As String = originalCompleteFieldName.ToLower

                                Dim partialFieldName As String =
                                    If(IsNothing(orderByField.ChildNodes.Item(1).Attributes("ColumnOrPropertyName")), completeFieldName,
                                        orderByField.ChildNodes.Item(1).Attributes("ColumnOrPropertyName").Value.ToLower)

                                Dim parserSortOrder As String = orderByField.Attributes("SortOrder").Value.ToLower
                                Dim sqlSortOrder As String = ""
                                If parserSortOrder <> "none" Then
                                    sqlSortOrder = If(parserSortOrder = "ascending", "ASC", "DESC")
                                End If
                                Dim orderByType As String = If(Not IsNothing(orderByField.ChildNodes.Item(1).Attributes("Type")), orderByField.ChildNodes.Item(1).Attributes("Type").Value.ToLower, "")

                                If Not elencoAlias.Contains(completeFieldName) AndAlso orderByType <> "integer" Then
            ...
            'Scrivi_LOG("", "", "Parametrizzatore.ParametrizzaOrderBy", "", String.Format("Time for parsing: {0} milliseconds", sw.ElapsedMilliseconds))
    ...
    Private Function Query_Con_Union(ByVal nodoOrderBy As XmlNode) As Boolean

        If IsNothing(nodoOrderBy) Then
            Return False
        End If

        Dim nodoPadre As XmlNode = nodoOrderBy.ParentNode
        If IsNothing(nodoPadre) Then
            Return False
        End If

        If IsNothing(nodoPadre.ChildNodes) Then
            Return False
        End If

        For Each cn As XmlNode In nodoPadre.ChildNodes
            If cn.Name.Contains("SqlBinaryQueryExpression") Then
                Dim unionAt As String = cn.Attributes("Operator").Value.ToLower
                If unionAt = "unionall" OrElse unionAt = "union" Then
                    Return True
                End If
            End If
        Next
    ...
    Private Function Query_Con_Distinct(ByVal nodoOrderBy As XmlNode) As Boolean
        ...
        Dim selectClause As XmlNodeList = nodoPadre.SelectNodes("./SqlQuerySpecification/SqlSelectClause[@IsDistinct='True']")
        If IsNothing(selectClause) Then
            Return False
        End If

        Return selectClause.Count >= 1

    End Function

    Private Function Ottieni_Lista_Campi_Select(ByVal nodoOrderBy As XmlNode) As List(Of String)
        ...
        Dim nodoQuerySpec As XmlNodeList = nodoPadre.ChildNodes
        If IsNothing(nodoQuerySpec) OrElse nodoQuerySpec.Count < 2 Then
            Return nomiCampi
        End If

        Dim nodiDiCampi As XmlNodeList = nodoQuerySpec.Item(1).SelectNodes(".//SqlSelectScalarExpression")
        If Not IsNothing(nodiDiCampi) AndAlso nodiDiCampi.Count > 0 Then
            For Each na As XmlNode In nodiDiCampi
                Dim re As XmlNodeList = na.SelectNodes(".//SqlScalarRefExpression")

                Dim cre As XmlNodeList = na.SelectNodes(".//SqlColumnRefExpression")

                For Each r In re
                    If Not IsNothing(r.Attributes("MultipartIdentifier")) Then
                        nomiCampi.Add(r.Attributes("MultipartIdentifier").Value.ToLower)
                    End If
                Next
    ...
    Private Function Ottieni_Alias_Campi(ByVal nodoOrderBy As XmlNode) As List(Of String)
        ...
        Dim nodiDiAlias As XmlNodeList = nodoQuerySpec.Item(1).SelectNodes(".//SqlSelectScalarExpression[@Alias]")
        If Not IsNothing(nodiDiAlias) AndAlso nodiDiAlias.Count > 0 Then
            For Each na As XmlNode In nodiDiAlias
                nomiAlias.Add(na.Attributes("Alias").Value.ToLower)
            Next
        End If
    ...
    Private Function ParametroEffettivo(Of T)(ByVal valore As Object) As T

        Return Convert.ChangeType(valore, GetType(T))

    End Function

    Private Sub ParamettroEffettivo(ByVal valoreStringa As String, ByRef p As SqlParameter)
    ...
    Private Function Agro_SQL_Save_Clausola_IN_Con_Parametri(ByVal clausolaIN As String, ByVal valoriStringa As Boolean, Optional ByVal injectionGuid As Guid = Nothing) As String
        ...
        For Each s As String In ins
            ...
            If valoriStringa Then
                valoreParametro = String.Format("{0}{1}{2}", nomenclatura.Item1, s.Replace("'", ""), nomenclatura.Item2)
...
#Region "ControllaInjection"
    ...
    Private Function ControllaInjection(ByVal query As String,
        ...
        'If (query.Contains(";") = True OrElse query.Contains("--") = True OrElse query.Contains("/*") = True OrElse Regex.Match(query, "\s*.*\s*from\s*.*\s*sys.*\s*where\s*.*").Success) Then

        '    Throw New UnauthorizedAccessException(errore & "Utilizzo di ; / del prefisso sys / di commenti relativamente sospetti")

        'End If

        If (query.Contains(";") OrElse query.Contains("--") OrElse query.Contains("/*")) Then

            Throw New UnauthorizedAccessException(errore & "Utilizzo di ; / di commenti relativamente sospetti")
    ...
    Private Sub ControlloOr(ByVal orExpressions As XmlNodeList, ByVal errore As String)
        ...
        For Each orNodes As XmlNode In orExpressions
            ...
            For Each orComparisonNodes As XmlNode In orComparisonFiltered
                ...
                If (((literalCount - topCount - subQCount) = 2 OrElse isLiteralInjection) OrElse (literalCount = 0 AndAlso String.Compare(refExpressionL, refExpressionR) = 0)) Then
...
#Region "Logging"
    ...
    Overloads Sub Logga(ByVal NomeRoutine As String,
                        ByVal MessaggioErrore As String,
                            Optional ByVal objParametri As AgronicaCoreParametri = Nothing,
                            Optional verificaInviaElasticSearch As Boolean = False) Implements IParametrizzatore.Logga
