'------------------------------------------------------------------------------
' <auto-generated>
'     Codice generato da un modello.
'
'     Le modifiche manuali a questo file potrebbero causare un comportamento imprevisto dell'applicazione.
'     Se il codice viene rigenerato, le modifiche manuali al file verranno sovrascritte.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic
Imports System.Runtime.Serialization    'MANUAL ADD

<Serializable()>
<DataContract(IsReference:=True)>
Partial Public Class NC_Dettagli

    'Simple Properties
    <DataMember()>
    Public Property ID_Dettaglio As Integer
    <DataMember()>
    Public Property PivaSuperUser As String
    <DataMember()>
    Public Property ID_NC As Integer
    <DataMember()>
    Public Property CodiceDettaglio As String
    <DataMember()>
    Public Property Utente As String
    <DataMember()>
    Public Property Data As Date
    <DataMember()>
    Public Property ID_Stato As Nullable(Of Integer)
    <DataMember()>
    Public Property ID_ListaAllegati As Nullable(Of Integer)
    <DataMember()>
    Public Property Descrizione As String
    <DataMember()>
    Public Property Note As String
    <DataMember()>
    Public Property DataChiusuraPrevista As Nullable(Of Date)
    <DataMember()>
    Public Property DataChiusuraEffettiva As Nullable(Of Date)
    <DataMember()>
    Public Property Responsabile As String
    <DataMember()>
    Public Property PersoneCoinvolte As String
    <DataMember()>
    Public Property inviato As Nullable(Of Short)
    <DataMember()>
    Public Property datainvio As Nullable(Of Date)
    <DataMember()>
    Public Property Data_Creazione As Date
    <DataMember()>
    Public Property Data_Modifica As Date
    <DataMember()>
    Public Property Username_Creazione As String
    <DataMember()>
    Public Property Username_Modifica As String
    <DataMember()>
    Public Property Validita_Inizio As Nullable(Of Date)
    <DataMember()>
    Public Property Validita_Fine As Nullable(Of Date)

End Class
