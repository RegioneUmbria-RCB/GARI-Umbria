'------------------------------------------------------------------------------
' <auto-generated>
'     Codice generato da un modello.
'
'     Le modifiche manuali a questo file potrebbero causare un comportamento imprevisto dell'applicazione.
'     Se il codice viene rigenerato, le modifiche manuali al file verranno sovrascritte.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic
Imports System.Runtime.Serialization    'MANUAL ADD

<Serializable()>
<DataContract(IsReference:=True)>
Partial Public Class Celle_Meteo

    'Simple Properties
    <DataMember()>
    Public Property CLASSE As Integer
    <DataMember()>
    Public Property CELLA As Integer
    <DataMember()>
    Public Property RIF As String
    <DataMember()>
    Public Property DESCR As String
    <DataMember()>
    Public Property X_UTM As Nullable(Of Double)
    <DataMember()>
    Public Property Y_UTM As Nullable(Of Double)
    <DataMember()>
    Public Property X_LON As Nullable(Of Double)
    <DataMember()>
    Public Property Y_LAT As Nullable(Of Double)
    <DataMember()>
    Public Property QUOTA As Nullable(Of Integer)
    <DataMember()>
    Public Property ATTIVA As Nullable(Of Integer)
    <DataMember()>
    Public Property DENOMINAZIONE As String
    <DataMember()>
    Public Property PROVINCIA As String
    <DataMember()>
    Public Property inviato As Nullable(Of Short)
    <DataMember()>
    Public Property datainvio As Nullable(Of Date)
    <DataMember()>
    Public Property Data_Creazione As Nullable(Of Date)
    <DataMember()>
    Public Property Data_Modifica As Nullable(Of Date)
    <DataMember()>
    Public Property Username_Creazione As String
    <DataMember()>
    Public Property Username_Modifica As String
    <DataMember()>
    Public Property Validita_Inizio As Nullable(Of Date)
    <DataMember()>
    Public Property Validita_Fine As Nullable(Of Date)

End Class
