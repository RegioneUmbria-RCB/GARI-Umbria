'------------------------------------------------------------------------------
' <auto-generated>
'     Codice generato da un modello.
'
'     Le modifiche manuali a questo file potrebbero causare un comportamento imprevisto dell'applicazione.
'     Se il codice viene rigenerato, le modifiche manuali al file verranno sovrascritte.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic
Imports System.Runtime.Serialization    'MANUAL ADD

<Serializable()>
<DataContract(IsReference:=True)>
Partial Public Class UMA_UF_Colture

    'Simple Properties
    <DataMember()>
    Public Property Occupazione_Cod As String
    <DataMember()>
    Public Property Destinazione_Cod As String
    <DataMember()>
    Public Property Uso_Cod As String
    <DataMember()>
    Public Property Qualita_Cod As String
    <DataMember()>
    Public Property UF_Ha As Nullable(Of Double)
    <DataMember()>
    Public Property UFL_Ha As Nullable(Of Double)
    <DataMember()>
    Public Property UFC_Ha As Nullable(Of Double)
    <DataMember()>
    Public Property UF_Ha_Irrigua As Nullable(Of Double)
    <DataMember()>
    Public Property UFL_Ha_Irrigua As Nullable(Of Double)
    <DataMember()>
    Public Property UFC_Ha_Irrigua As Nullable(Of Double)
    <DataMember()>
    Public Property inviato As Nullable(Of Short)
    <DataMember()>
    Public Property datainvio As Nullable(Of Date)
    <DataMember()>
    Public Property Data_Creazione As Nullable(Of Date)
    <DataMember()>
    Public Property Data_Modifica As Nullable(Of Date)
    <DataMember()>
    Public Property Username_Creazione As String
    <DataMember()>
    Public Property Username_Modifica As String
    <DataMember()>
    Public Property Validita_Inizio As Date
    <DataMember()>
    Public Property Validita_Fine As Nullable(Of Date)

End Class
