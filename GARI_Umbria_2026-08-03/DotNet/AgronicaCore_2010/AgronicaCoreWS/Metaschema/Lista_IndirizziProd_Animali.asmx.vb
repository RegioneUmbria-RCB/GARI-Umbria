...
<ToolboxItem(False)>
Public Class Lista_IndirizziProd_Animali
	Inherits System.Web.Services.WebService

	<WebMethod(EnableSession:=True)>
	<Script.Services.ScriptMethod()>
	Public Function get_Lista_IndirizziProd_Animali_NG(InData As CoreWS_Generic(Of get_Lista_IndirizziProd_Animali)) As RispostaStandard
		...
		Dim indProdAnimali_R As New Lista_IndProd_Animali_R
		Try
			...
			Dim strP As String = "get_Lista_IndirizziProd_Animali_" & InData.InData.gen_cod & "_" & InData.InData.spe_cod & "_" & linguaCodiceISO

			If IsNothing(HttpContext.Current.Cache(strP)) Then
				Dim listaIndProd = indProdAnimali_R.Leggi(genCod, speCod, -1, enumSelezioneVariabile.Selezione_TabellaDatiMinimi, "", "", objP_Server)

				Dim strRisp = Newtonsoft.Json.JsonConvert.SerializeObject(listaIndProd)
	...
	<WebMethod(EnableSession:=True)>
	<Script.Services.ScriptMethod()>
	Public Function get_Lista_IndirizziProd_Animali(gen_cod As Integer, spe_cod As Integer, objP_server As String, objP_utenti As String) As RispostaStandard
		Dim r As New RispostaStandard
		...
		Try
            ...
            Dim strP As String = "get_Lista_IndirizziProd_Animali_" & gen_cod & "_" & spe_cod & "_" & linguaCodiceISO

            If IsNothing(HttpContext.Current.Cache(strP)) Then
				Dim listaIndProd = indProdAnimali_R.Leggi(gen_cod, spe_cod, -1, enumSelezioneVariabile.Selezione_TabellaDatiMinimi, "", "", objParams_Server)
