        ...
        Try
            ...
            Select Case xSelezioneVariabile
                ...
                Case enumSelezioneVariabile.Selezione_JoinDescrizioni
                    'Tutte le colonne con join su viste UMA_Allevamenti, Carburanti e UMA_Configurazione_Allevamenti
                    ...
                    strSql.AppendLine(" config.ufl_min, config.ufl_max, config.ufc_min, config.ufc_max, ")
                    strSql.AppendLine(" CASE ")
                    strSql.AppendLine(" WHEN ( ISNULL(config.ufl_min,0)>0 AND ISNULL(config.ufl_max,0)>0 ) OR ")
                    strSql.AppendLine("      ( ISNULL(config.ufc_min,0)>0 AND ISNULL(config.ufc_max,0)>0 ) THEN 'Si' ")
                    strSql.AppendLine(" ELSE 'No' ")
                    strSql.AppendLine(" END As Incluso_nel_Calcolo ")
                    strSql.AppendLine("FROM UMA_Richieste_Allevamenti")
                    'Join UMA_Allevamenti
                    ...
                    'Join UMA_Configurazione_Allevamenti
                    strSql.AppendLine("INNER JOIN UMA_Richieste_Testata testata")
                    strSql.AppendLine("   ON testata.Piva_SuperUser = UMA_Richieste_Allevamenti.Piva_SuperUser ")
                    strSql.AppendLine("  AND testata.Piva = UMA_Richieste_Allevamenti.Piva ")
                    strSql.AppendLine("  AND testata.Richiesta_Cod = UMA_Richieste_Allevamenti.Richiesta_Cod ")
                    strSql.AppendLine(" LEFT JOIN UMA_Configurazione_Allevamenti config")
                    strSql.AppendLine("   ON config.UMA_All_Cod = UMA_Richieste_Allevamenti.UMA_All_Cod")
                    strSql.AppendLine("  AND config.Validita_Inizio <= testata.Validita_Fine ")
                    strSql.AppendLine("  AND config.Validita_Fine >= testata.Validita_Inizio ")
                    strSql.AppendLine("WHERE 1=1")
                ...
                Case enumSelezioneVariabile.Selezione_JoinCompleta
                    ...
                    strSql.AppendLine("FROM UMA_Richieste_Allevamenti")
                    strSql.AppendLine("INNER JOIN UMA_Richieste_Testata")
                    strSql.AppendLine(" ON UMA_Richieste_Testata.Piva_SuperUser = UMA_Richieste_Allevamenti.Piva_SuperUser ")
                    strSql.AppendLine(" AND UMA_Richieste_Testata.Piva = UMA_Richieste_Allevamenti.Piva ")
                    strSql.AppendLine(" AND UMA_Richieste_Testata.Richiesta_Cod = UMA_Richieste_Allevamenti.Richiesta_Cod ")
                    strSql.AppendLine("INNER JOIN UMA_Allevamenti")
                    strSql.AppendLine(" ON UMA_Richieste_Allevamenti.UMA_All_Cod = Uma_Allevamenti.UMA_All_Cod")
                    ...
                    strSql.AppendLine(" AND UMA_Configurazione_Allevamenti.Validita_Inizio <= UMA_Richieste_Testata.Validita_Fine ")
                    strSql.AppendLine(" AND UMA_Configurazione_Allevamenti.Validita_Fine >= UMA_Richieste_Testata.Validita_Inizio ")
                    strSql.AppendLine("INNER JOIN Carburanti")
                    strSql.AppendLine(" ON UMA_Richieste_Allevamenti.Tipo_Carburante = Carburanti.Car_Cod")
                    strSql.AppendLine("WHERE 1=1")
            End Select
            ...
            If piva <> "" Then
                strSql.AppendLine("AND UMA_Richieste_Allevamenti.Piva = '" & Agro_SQL_SaveText(piva) & "'")
            End If
            If umaAllCod <> "" Then
                strSql.AppendLine("AND UMA_Richieste_Allevamenti.UMA_All_Cod = '" & Agro_SQL_SaveText(umaAllCod) & "'")
            End If
...
Public Class UMA_Richieste_Allevamenti_W
    ...
    Public Function Aggiorna_Allevamenti(
        ...
        Dim nomeRoutine As String = "AgronicaCoreUmaDAL.UMA_Richieste_Allevamenti_W.Aggiorna()"
