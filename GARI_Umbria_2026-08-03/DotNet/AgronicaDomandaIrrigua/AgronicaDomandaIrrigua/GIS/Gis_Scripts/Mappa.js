...
var mappa = {
        ...
        if (!selectedShape_testo.indexOf("§") > 0) {
            ...
            if (r.extra_info) {
                for (var curInfo in r.extra_info) {
                    //utility.log(curInfo);
                    msg += "<i>" + curInfo + ': ' + r.extra_info[curInfo] + "</i><br />";
                }
        ...
        if ($("#spanGiasPalmDettagli").text() != msgNascondiDet) {
            ...
            ajaxAgronica(indirizzohttp + "/GiasPalmDettagli", JSON.stringify({ piva: piva, sa_cod: sa_cod, id: id }),
                function (risposta) {
                    ...
                    //i18n__
                    msg += "<br /><i style = 'font-size: medium; font-weight: bold'>Dettagli Gias Palm:</i> <br /><br />";
                    msg += "<b>Perimetro</b>: " + r.giaspalm_dettagli.Perimetro + "<br />";
                    msg += "<b>Sup. Rilevata in Campo</b>: " + r.giaspalm_dettagli.Area + "<br />";
                    ...
                    for (var curInfo in det) {
                        ...
                        if (i > 0)
                            msg += "<i>P. " + i.toString() + '</i>: <br />';

                        //info del punto
                        for (var curInfoPunto in det[curInfo]) {
                            if (curInfoPunto != "NMEA") {
                                msg += "<b>" + curInfoPunto + '</b>: ' + det[curInfo][curInfoPunto] + "<br />";
                            }
                        }
                        msg += "</i><br />"
                        i++;
    ...
    , RenderizzaShape: function () {
        ...
        RenderizzaShape_LivelloZoom = Zoom;
    ...
    ShowHideEtichette: function (showFlag) {
        ...
        for (l = 0; l < this.Livelli.length; l++) {
...
function PolyLineFill(cfgObj) {
    //let bounds = new google.maps.LatLngBounds();
    //for (let i = 0; i < cfgObj.poly.length; i++) {
    //    bounds.extend(cfgObj.poly[i]);
    //}
...
PolyLineFill.prototype.calcBounds = function () {
    ...
    for (let i = 0; i < this.poly_.length; i++) {
        this.bounds_.extend(this.poly_[i]);
    }
}

PolyLineFill.prototype.setPath = function (index, obj, op) {
    if (op === 'replace') {
        this.poly_[index] = obj;
    } else if (op === 'insert') {
...
//function DraggableOverlay(map, poly) {
...
//    this.bounds_ = new google.maps.LatLngBounds();
//    let that = this;
//    this.poly_.forEach(function (elem, idx) {
//        that.bounds_.extend(elem);
//    }); 
...
//    //add polygon to the div
//    let p = document.createElementNS(svgns, "polygon");
//    p.setAttributeNS(null, 'fill', this.fill_);
//    p.setAttributeNS(null, 'stroke', this.fill_);
//    p.setAttributeNS(null, 'stroke-width', '1');
//    p.setAttributeNS(null, "stroke-opacity", this.opacity_);
//    p.setAttributeNS(null, 'pointer-events', 'all');
//    //set a reference to this element;
//    this.polysvg_ = p;
//    svg.appendChild(p);
...
//    let that = this;
//    //this.moveHandler_ = google.maps.event.addDomListener(this.getMap().getDiv(), 'mousemove', function (e) {
//    this.moveHandler_ = google.maps.event.addDomListener(this.div_, 'mousemove', function (e) {

//        that.movement.x += e.movementX;
//        that.movement.y += e.movementY; 
//        that.draw();
...
//        //if (that.origin_ !== null) {
//        //    oClientX = that.origin_.clientX;
//        //    oClientY = that.origin_.clientY;
//        //}
//        //if (that.position_ !== null) {
//        //    let pos = that.getProjection().fromLatLngToDivPixel(that.position_);
//        //    posX = pos.x;
//        //    posY = pos.y;
//        //}
//        //let left = oClientX - e.clientX;
//        //let top = oClientY - e.clientY;
//        //let latLng = that.getProjection().fromDivPixelToLatLng(new google.maps.Point(posX - left, posY - top));
//        //that.position_ = latLng;
//        //that.origin_ = e;

//        //console.log(that.position_.lat() + " " + that.position_.lng());
//        ////that.draw();
//    });
//}
...
//    // Resize the image's DIV to fit the indicated dimensions.
//    let div = this.div_;
//    div.style.left = (this.movement.x + sw.x) + 'px';
//    div.style.top = (this.movement.y + ne.y) + 'px';
//    div.style.width = (ne.x - sw.x) + 'px';
//    div.style.height = (sw.y - ne.y) + 'px';
...
//    let points = "";
//    this.poly_.forEach(function (elem, idx) {
//        let point = overlayProjection.fromLatLngToDivPixel(elem);
//        if (idx == 0) {
//            points += (point.x - sw.x) + ", " + (point.y - ne.y);
//        } else {
//            points += " " + (point.x - sw.x) + ", " + (point.y - ne.y);
...
//    this.polysvg_.setAttributeNS(null, "points", points);
//}
...
//    google.maps.event.removeListener(this.moveHandler_);
//}
