...
// ATTENZIONE!!!
//--------------------------------------------------------------------------------
// Se si modifica la firma di una funzione dei WebService richiamati a seguire,
// occorre verificare le chiamate sulle soluzioni Agenda e Stampe.
//--------------------------------------------------------------------------------
...
var indirizzohttp_Leggi_Tabelle_GHG_WS = "GHG/GHG_WS.asmx";
...
var indirizzoHttp_Attivita_WS = "Contab/Attivita.asmx";
...
function GetUrlLetturaTabelleGestionali() {
    ...
    if (typeof pathCoreWS !== "undefined") {
        if (pathCoreWS !== undefined && pathCoreWS.includes(window.location.protocol)) {
            url = "";
        }
    } else {
        pathCoreWS = "";
...
function RicercaContattiDocumentoAsync(flagVuoto, piva, tipoRapporto, dataValidita, filtraDataSuDb, filtraSoloValidi, cercaSoloValidi,
    ...
    return new Promise(function (resolve) {
        if (elencoContatti[tipoRapporto] === null || elencoContatti[tipoRapporto] === undefined) {
            ...
            ajaxAgronica(GetUrlLetturaTabelleGestionali() + "Anagrafica/Contatti.asmx/LeggiRapportoDocumenti",
                ...
                function (risposta) {
                    ...
                    if (filtraDataSuDb === false && dataValidita !== null) {
                        elencoContatti[tipoRapporto] = 
                            elencoContatti[tipoRapporto].map(function (x) {
                                return {
                                    ...x,
                                    parsedValidita_Inizio: kendo.parseDate(x.Validita_Inizio),
                                    parsedValidita_Fine: kendo.parseDate(x.Validita_Fine)
                                };
                            });
                        resolve(elencoContatti[tipoRapporto]
                            .filter(function (x) {
                                return x.parsedValidita_Inizio <= dataValidita && x.parsedValidita_Fine >= dataValidita;
                            }) 
                        );
            ...
            if (filtraDataSuDb === false && dataValidita !== null) {
                if (elencoContatti[tipoRapporto].length > 0 && elencoContatti[tipoRapporto][0].parsedValidita_Inizio) {
                    resolve(elencoContatti[tipoRapporto].filter(function (x) {
                        return x.parsedValidita_Inizio <= dataValidita && x.parsedValidita_Fine >= dataValidita;
                    }));
                } else {
...
function RicercaContattiDocumentoSync(flagVuoto, piva, tipoRapporto, filtraSoloValidi, cercaSoloValidi, includiIndirizzo,
    accettazioneConGerarchia, pivaPadreGerarchia, testoRicerca, codRisUm, codContatto, gestioneWaitFrame, checkRaccolte, dataFineRaccolte, dataValidita) {

    if (dataValidita === undefined)
        dataValidita = null;
...
function RicercaParametriQualitativiFiltroSpecieVarieta(flagVuoto, piva, modulo_generazione, veg_cod, cul_cod) {
    ...
    if (veg_cod === undefined || veg_cod === null)
        veg_cod = 0;

    if (cul_cod === undefined || cul_cod === null)
        cul_cod = 0;
...
function RicercaParametriIndici(flagVuoto, piva, tipoindice) {

    let parametriQualitativiIndici = [];
    if (flagVuoto === undefined || flagVuoto === null)
        flagVuoto = true;

    var param = kendo.stringify({ objP_server: objP_server, objP_utenti: objP_utenti, piva: piva, id_indice: 0, tipoindice: tipoindice });

    ajaxAgronicaSync(GetUrlLetturaTabelleGestionali() + indirizzohttp_Leggi_Tabelle_FF_WS + "/LeggiParametriIndici",
        param, false,
        function (risposta) {
            if (risposta.RispostaStringa !== "") {
                ...
                parametriQualitativiIndici = risp;
            }
        }, null);

    return parametriQualitativiIndici;
}


function CalcoloStandardFactor(qty_trasporto_attuale, carCod, udmCod, direttivaCod, codiceStato) {

    var valore = 0;
   
    var param = kendo.stringify({
        qty_trasporto_attuale: qty_trasporto_attuale,
        carCod: carCod,
        udmCod: udmCod,
        direttivaCod: direttivaCod,
        codiceStato: codiceStato,
        xfiltroaggiuntivo: "",
        objP_server: objP_server
    });

    ajaxAgronicaSync(GetUrlLetturaTabelleGestionali() + indirizzohttp_Leggi_Tabelle_GHG_WS + "/CalcoloStandard_Factor",
        param, false,
        function (risposta) {
           valore = risposta.RispostaStringa;          
        }, null);
...
function RicercaDDLParametriIndice(piva, id_indice, tipocampo, elenco_tipo, elenco_cod, data, filtroaggiuntivo) {
    ...
    ajaxAgronicaSync(GetUrlLetturaTabelleGestionali() + indirizzohttp_Leggi_Tabelle_FF_WS + "/LeggiParametriIndiciDettagli",
        param, false,
        function (risposta) {
            var risp = JSON.parse(risposta.RispostaStringa);
            ...
            function (risposta) {
                ...
                if (Array.isArray(risp)) {
                    if (leggiTerrenoNudo)
                        ...
                        elencoSpecie = risp.filter(function (x) { return (x.Veg_Cod !== "0") });
                }
            }, null);
...
function RicercaFornitoriAsync(flagVuoto, piva) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (elencoFornitori === null) {
            ...
            ajaxAgronica(GetUrlLetturaTabelleGestionali() + indirizzohttp_Contatti + "/LeggiFornitori_FF",
                param,
                function (risposta) {
                    var risp = JSON.parse(risposta.RispostaStringa);
                    if (flagVuoto === true) {
...
function RicercaGiacenze(Data_Movimento, Tipo_Aggregazione, SoloCampiApp, piva, Sa_Cod, Tipo_Fabbricato_Cod, Fabbricato_Cod, Elem_Cod, Pro_Cod, Mat_Cod, Lotto, isFreshAndFood, flag_QtaNoZero, flagVuoto) {
    ...
    var param =
        ...
        " pro_cod: " + Pro_Cod + "," +
        " mat_cod: " + Mat_Cod + "," +
        " lotto: '" + Lotto + "'," + //Parametro non usato dall'asmx
        " Data_Movimento_Str: '" + Data_Movimento + "'," +
...
function OttieniEECAjax(piva, elemCod, matCod, data) {
    let risp = undefined;
    let params = kendo.stringify({
        "objP_server": objP_server,
        "piva": piva,
        "elemCod": elemCod,
        "matCod": matCod,
    ...
    ajaxAgronicaSync(GetUrlLetturaTabelleGestionali() + indirizzohttp_Leggi_Tabelle_GHG_WS + "/OttieniEECPresunto",
        params, false,
        function (risposta) {
            risp = parseFloat(risposta.RispostaStringa)
        }, null);

    return risp;
}

function OttieniParamsQualGHGAjax(agendas) {
    let risp = undefined;
    let params = kendo.stringify({
        "agendas": agendas,
        "objP_super_server": objP_super_server,
    ...
    ajaxAgronicaSync(GetUrlLetturaTabelleGestionali() + indirizzohttp_Leggi_Tabelle_GHG_WS + "/OttieniParamQualDaRaccolte",
        params, false,
        function (risposta) {
            risp = JSON.parse(risposta.RispostaStringa)
        }, null);
