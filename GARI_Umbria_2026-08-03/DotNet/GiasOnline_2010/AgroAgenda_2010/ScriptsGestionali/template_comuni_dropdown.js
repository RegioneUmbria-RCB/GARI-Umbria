...
var paramQualSpeVarPrec = new Object();
...
function paramQual_Template(container, options) {
    ...
    for (var i = 0; i < paramQualGestiti.length; i++) {
        ...
        if (paramQualGestiti[i].Tabella_ID !== 0) {
            ...
            const colonnaParamQualTipoCod = "FF_" + paramQualGestiti[i].Tabella_Cod_Des + "_Tipo_Cod";
            const colonnaParamQualSigla = "FF_" + paramQualGestiti[i].Tabella_Cod_Des + "_Sigla";
            const colonnaParamQualDescrizione = "FF_" + paramQualGestiti[i].Tabella_Cod_Des + "_Descrizione";

            if (options.field === colonnaParamQualSigla || options.field === colonnaParamQualDescrizione) {
                tabella_ID = kendo.parseInt(paramQualGestiti[i].Tabella_ID);
                campoCodice = colonnaParamQualTipoCod;
            }
...
function paramQual_SpeVar_Template(container, options) {

    //const nomeFunzione = "paramQual_SpeVar_Template_" + options.field;

    //console.time(nomeFunzione);

    let w_modulo_anagrafe_log = 0;
    if (options.model.Cat_Cod === 210)
        w_modulo_anagrafe_log = Modulo_FreshFood;
    else
        if (options.model.Cat_Cod === 310)
            w_modulo_anagrafe_log = Modulo_Zoo;

    if (paramQualSpeVarPrec.Piva === undefined ||
        paramQualSpeVarPrec.Piva !== $(cIdPiva).val() ||
        paramQualSpeVarPrec.ModuloAnagrafe !== w_modulo_anagrafe_log ||
        paramQualSpeVarPrec.Veg_Cod !== options.model.Veg_Cod ||
        paramQualSpeVarPrec.Cul_Cod !== options.model.Cul_Cod) {
        paramQualGestiti = RicercaParametriQualitativiFiltroSpecieVarieta(true, $(cIdPiva).val(), w_modulo_anagrafe_log, options.model.Veg_Cod, options.model.Cul_Cod);
    } else {
        paramQualGestiti = paramQualSpeVarPrec.paramQualGestiti;
    }
    ...
    paramQualSpeVarPrec.Veg_Cod = options.model.Veg_Cod;
    paramQualSpeVarPrec.Cul_Cod = options.model.Cul_Cod;
    paramQualSpeVarPrec.paramQualGestiti = paramQualGestiti;
    ...
    if (tabella_ID !== 0 && campoCodice !== "") {

        var valoriParametriQualitativiFF = RicercaValoriParametriQualitativiFiltratiSpecieVarieta(tabella_ID, $(cIdPiva).val(), true, options.model.Veg_Cod, options.model.Cul_Cod); //TODO: GIULIA - ELIMINA ANCHE DA QUI!!!
