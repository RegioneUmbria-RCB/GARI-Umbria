...
function LeggiValoriParametriIndiciDettagli(options) {
    var elencoValori = this.data.elencoValori;
    options.success(elencoValori);
}
...
function creaArrayCampionature(rigaKendo, paramQualGestiti) {
    var arrParams = [];
    ...
    for (var chiave in rigaKendo) {
        ...
        if (rigaKendo.hasOwnProperty(chiave)) {
            ...
            if (chiave.startsWith("FF_")) {
                ...
                let tipo_param = null;
                for (var iPar = 0; iPar < paramQualGestiti.length; iPar++) {
                    if (paramQualGestiti[iPar].Tabella_ID !== 0 &&
                        (chiave === "FF_" + paramQualGestiti[iPar].Tabella_Cod_Des + "_Tipo_Cod" ||
                            chiave === "FF_" + paramQualGestiti[iPar].Tabella_Cod_Des + "_Val_Cod")) {

                        tipo_param = paramQualGestiti[iPar].Tipo;
                ...
                if (chiave.endsWith("_Tipo_Cod") || chiave.endsWith("_Val_Cod")) {

                    var valCod = rigaKendo[chiave];
                    ...
                    switch (tipo_param) {
                        ...
                        case 5:
                            if (valCod !== null) {
                                valCod = formattedReverseDate(valCod, "-")
                            }
                            break;
...
function creaNewRowDiv(id, className) {
    var newRowDiv = document.createElement("div");
    newRowDiv.className = "row";

    if (className !== null && className !== undefined)
        newRowDiv.className = className;
...
function creaLabel(id, testo, classe, labelFor, miostile) {
...
function creaInputGenerico(id, name, classe, functionChange, required, miostile, dataAttribute_idparamqual) {
    ...
    if (dataAttribute_idparamqual !== null && dataAttribute_idparamqual !== undefined)
        inputGenerico.dataset.idparamqual = dataAttribute_idparamqual;
...
function checkAntiVirus_isON(objP_super_server) {
    ...
    AntiVirus_On = Boolean(parseInt(AntiVirus_On))
    return (AntiVirus_On);
}

function funzioneDaChiamareDopoCambiLayoutGriglia(grid, funcColumn, funcReorder, funcFilter, funcGroup, funcSort) {
    grid.bind("columnReorder", funcReorder);
    grid.bind("columnHide", funcColumn);
    grid.bind("columnShow", funcColumn);
    grid.bind("columnStick", funcColumn);
    grid.bind("columnUnstick", funcColumn);
    grid.bind("filter", funcFilter);
    grid.bind("group", funcGroup);
    grid.bind("sort", funcSort);
    grid.bind("columnLock", funcColumn);
    grid.bind("columnUnlock", funcColumn);
} 