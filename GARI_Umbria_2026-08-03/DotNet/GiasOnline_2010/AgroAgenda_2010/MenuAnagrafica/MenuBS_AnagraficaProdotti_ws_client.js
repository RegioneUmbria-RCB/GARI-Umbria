var indirizzohttpPaginaProdotto = "./MenuBS_AnagraficaProdotti.aspx";
...
function Carica_prodotto_UC_ddlCodIvaCompensazione(options) {
    ...
    for (var i = 1; i < risp.length; i++) {
        ElencoIvaComp[i] = {
            Cod: risp[i].Cod, Descrizione: risp[i].Codice + " - " + risp[i].Descrizione + "-" + Traduzione(menuBSAnagraficaResx, "IvaOrdinariaBreve", "Iva Ord.") + " " + risp[i].Aliquota_Ordinaria + " % - " + Traduzione(menuBSAnagraficaResx, "IvaCompensazioneBreve", "Iva Comp.") + " " + risp[i].Aliquota_Compensativa + " %" };
    }
...
function Carica_ddl_set(options) {
    let ElencoSet = [];
    ElencoSet.push({ peso_set_cod: 0, peso_set_des: Traduzione(menuBSAnagraficaResx, "PesoNominaleNonModificabile", "Peso Nominale Non Modificabile") });
    ElencoSet.push({ peso_set_cod: 1, peso_set_des: Traduzione(menuBSAnagraficaResx, "PesoNominaleModificabile", "Peso Nominale Modificabile") });
    ElencoSet.push({ peso_set_cod: 2, peso_set_des: Traduzione(menuBSAnagraficaResx, "PesoModificabileEConsentiSfuso", "Peso Modificabile e Consenti Sfuso") });
...
function Carica_ddl_peso(options) {
    ...
    if (elem_cod === categorieProdotti.BENI_CONFEZIONAMENTO_ANIMALI || elem_cod === categorieProdotti.BENI_CONFEZIONAMENTO_VEGETALI) {
        ElencoPeso.push({ tipo_peso_cod: enum_TipoPeso.Peso_Netto, tipo_peso_des: Traduzione(menuBSAnagraficaResx, "Capacità", "Capacità:") });
    }
    else {
        ElencoPeso.push({ tipo_peso_cod: enum_TipoPeso.Peso_Netto, tipo_peso_des: Traduzione(menuBSAnagraficaResx, "PesoNettoNominaleKg", "Peso Netto Nominale Kg:") });
        ElencoPeso.push({ tipo_peso_cod: -1, tipo_peso_des: Traduzione(menuBSAnagraficaResx, "CapacitàNominaleLitri", "Capacità Nominale Litri:") });
    }
...
function Carica_ddl_Confezione_Base(options) {
    ...
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_NESSUNO, base_des: Traduzione(menuBSAnagraficaResx, "NessunAutomatismo", "Nessun Automatismo") });
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_CONFEZIONI_FF, base_des: Traduzione(menuBSAnagraficaResx, "Confezione", "Confezione") });
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_CONTENITORI_FF, base_des: Traduzione(menuBSAnagraficaResx, "Contenitore", "Contenitore") });
    ElencoConfezioneBase.push({ base_cod: enum_OmniTabelle.ot_IMBALLAGGI_FF, base_des: Traduzione(menuBSAnagraficaResx, "Imballaggio", "Imballaggio") });
...
function Carica_Tipo_Default(options) {
    ...
    ajaxAgronicaSync(indirizzohttpPaginaProdotto + "/Leggi_XddlTipo_Default",
        ...
        function (risposta) {
            let risp = JSON.parse(risposta.RispostaStringa);
            var objVuoto = { "Codice_Generazione": 0, "Descrizione": Traduzione(menuBSAnagraficaResx, "Altro", "Altro") };
            risp.unshift(objVuoto);
            options.success(risp);
        },
        null);
...
function RiempiMultiseSpecieDettagliBeniConf(options) {
    ...
    if (elencoSpecie !== null && elencoSpecie !== undefined) {
        ...
        if (index !== null) {
            ...
            elencoSpecie[index].Veg_Des = Traduzione(menuBSAnagraficaResx, "TutteLeSpecieVegetali", "Tutte le Specie Vegetali");
        }
...
function RiempiMultiseVarietaDettagliBeniConf(options) {
    ...
    if (elencoVarieta !== null && elencoVarieta !== undefined) {
        ...
        if (index !== null) {
            ...
            elencoVarieta[index].Cul_Des = Traduzione(menuBSAnagraficaResx, "TutteLeVarietàColturali", "Tutte le Varietà Colturali");
        }
...
function Carica_ddlProduzionePropria(options) {
    ...
    ElencoProduzioni.push({ Produzione_Cod: enum_Produzione_Cod.Prodotto_Commercializzato, Produzione_Des: Traduzione(menuBSAnagraficaResx, "ProdottoCommercializzato", "Prodotto Commercializzato") });
    ElencoProduzioni.push({ Produzione_Cod: enum_Produzione_Cod.Produzione_Propria, Produzione_Des: Traduzione(menuBSAnagraficaResx, "ProduzionePropria", "Produzione Propria") });
...
function Carica_ddlPesoEgalizzato(options) {
    ...
    ElencoPesi.push({ Peso_Cod: 1, Peso_Des: Traduzione(menuBSAnagraficaResx, "Si", "Sì") });
    ElencoPesi.push({ Peso_Cod: 0, Peso_Des: Traduzione(menuBSAnagraficaResx, "No", "No") });
                    ...
                    if (cal_cod !== 0) {
                        ...
                        if (mat_cod_ref !== parseInt(Get_KendoDDLValue("ddl_prodotto_UC_Prodottobase"))) {
                            if (Prodotto_Movimentato === true) {
                                ...
                                let Msg_Errore = Traduzione(menuBSAnagraficaResx, "ProdottoBaseRiferimentoNonCambiabile", "Il Prodotto Base di Riferimento non può essere cambiato perchè il prodotto risulta già movimentato");
    ...
    if (controlliPrimaDelSubmitOk) {
        ajaxAgronicaSync(indirizzohttpPaginaProdotto + "/Submit",
            ...
            function (risposta) {
                ...
                kendo.alert(indirizzohttpPaginaProdotto);
                kendo.alert(Traduzione(menuBSAnagraficaResx, "SalvataggioAvvenutoConSuccesso", "Salvataggio avvenuto con successo"));
...
function Cancella_Prodotto_Effettivo(rigaDaCancellare,materiaprima) {
    ...
    ajaxAgronica("../Anagrafica/Prodotto_Edit.aspx/Cancella_Prodotto",
        ...
        function (risposta) {
            ...
            if (materiaprima === true) {
                kendo.alert(Traduzione(menuBSAnagraficaResx, "ProdottoCancellatoCorrettamente", "Prodotto cancellato correttamente"));
            }
            else {
                kendo.alert(Traduzione(menuBSAnagraficaResx, "StoricoPrezziEDatiContabilitaCancellatiCorrettamente", "'Storico Prezzi' e 'Dati Contabilità' cancellati correttamente"));
            }
