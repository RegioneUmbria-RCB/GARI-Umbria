...
var JsnRegolamento = [
    { text: "Tutti", value: "0" },
    { text: "Convenzionale", value: "1" },
    { text: "Biologio", value: "4" }
];

var JsnVisibilita = [
    { text: "Tutti", value: "-99" },
    { text: "Privato", value: "0" },
    { text: "Pubblico", value: "-1" }
];
...
var indirizzohttp = "./MenuBS_AnagraficaProdotti.aspx";
        ...
        buttons: {
            ...
            Ok: {
                ...
                callback: function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: './MenuBS_AnagraficaProdotti.aspx/DeleteElemento',
...
function AggiornaDati(keys) {
    ...
    $.ajax({
        type: 'POST',
        url: './MenuBS_AnagraficaProdotti.aspx/AggiornaDati',
...
function NuovoElemento() {
    ...
    tipoNuovo.disableSelection = false;
...
function resetFiltrototale() {
    Prodotto_Edit_UC_DocReady_Lista_Anagrafica()
}
...
function gotoNewElement(tipo, chiave) {
    ...
    ajaxAgronicaSync(
        './MenuBS_AnagraficaProdotti.aspx/gotoNewElement',
...
function GotToFiltrino() {
    ...
    $.ajax({
        type: 'POST',
        url: './MenuBS_AnagraficaProdotti.aspx/GetFiltrino',
        data: "{}",
        contentType: 'application/json; charset=utf-8',
        cache: false,
        dataType: 'json', async: true,
...
function kendoProdotto_inizializza(divKendo, keys) {
    ...
    var templateModificaProdotto = "<span class='fa fa-pencil-square-o fa-2x edit_elem' title='" + Traduzione(menuBSAnagraficaResx, "ModificaProdotto", "Modifica Prodotto") + "' onclick=modificaProdotto(this.closest('tr'),this.closest('.k-grid'))></span>";

    var templateEliminaProdotto = "<span class='fa fa-trash-o fa-2x del_elem' title='" + Traduzione(menuBSAnagraficaResx, "EliminaProdotto", "Elimina Prodotto") + "' onclick=eliminaProdotto(this.closest('tr'),this.closest('.k-grid'))></span>";

    var templateVisualizzaProdotto = "<span class='fa fa-info fa-2x info_elem' title='" + Traduzione(menuBSAnagraficaResx, "VisualizzaProdotto", "Visualizza Prodotto") + "' onclick=infoProdotto(this.closest('tr'),this.closest('.k-grid'),0)></span>";

    var templateDuplicaProdotto = "<span class='fa fa-files-o fa-2x copia_elem' title='" + Traduzione(menuBSAnagraficaResx, "DuplicaProdotto", "Duplica Prodotto") + "' onclick=duplicaProdotto(this.closest('tr'),this.closest('.k-grid'),0)></span>";
                    ...
                    [
                        {
                            ...
                            , visible: function (dataItem) { return permesso_prodotti_read; }
                        },
                        {
                            template: templateModificaProdotto
                            , visible: function (dataItem) {
                                return nascondiBottoniProdotto_btnModifica(dataItem);
                            }
                        },

                        {
                            template: templateDuplicaProdotto
                            , visible: function (dataItem) {
                                return nascondiBottoniProdotto_btnDuplica(dataItem);
                            }
                        },
                        {
                            template: templateEliminaProdotto
                            , visible: function (dataItem) {
                                return nascondiBottoniProdotto_btnCancella(dataItem);
                            }
                        }
                    ]
                , title: "Azioni", width: "200px"
            }
        ]
...
function nascondiBottoniProdotto_btnDuplica(dataItem) {
...
function nascondiBottoniProdotto_btnCancella(dataItem) {
...
function nascondiBottoniProdotto_btnModifica(dataItem) {
...
function kReadValorizzazioneProdotto_col() {
    ...
    for (let c of columns) {
        ...
        if (c.field === "NomeComune") {
            c.title = Traduzione(menuBSAnagraficaResx, "CategoriaProdotto", "Categoria Prodotto");
            break;
        }
...
//function resetFiltrototale() {
//    alert($('#txt_descrizione_prodotto').value);
...
function caricaGrigliaProdotto(keys) {
    ...
    if (FiltroMinCaratteriPerRicerca() &&
        ...
        kendo.alert(Traduzione(menuBSAnagraficaResx, "InserireAlmenoTreCaratteriProdotto", "Inserire almeno tre caratteri della Descrizione/Cod. Articolo del Prodotto."));
    }
    else {
...
async function modificaProdotto(tr_elem, grid_elem) {
    ...
    $.ajax({
        type: 'POST',
        url: './MenuBs_AnagraficaProdotti.aspx/EditProdotto',
        data: param,
        contentType: 'application/json; charset=utf-8',
        cache: false,
        dataType: 'json', async: true,
...
async function infoProdotto(tr_elem, grid_elem) {
    ...
    $.ajax({
        type: 'POST',
        url: './MenuBs_AnagraficaProdotti.aspx/InfoProdotto',
        data: param,
        contentType: 'application/json; charset=utf-8',
        cache: false,
        dataType: 'json', async: true,
...
async function eliminaProdotto(tr_elem, grid_elem) {
    ...
    if (permessoModifica) {
        ...
        if (mat_prima === true) {
            ...
            if (movimentato === true) {
                kendo.alert(Traduzione(menuBSAnagraficaResx, "ProdottoMovimentatoNonCancellabile", "Il Prodotto è già movimentato non è possibile cancellarlo"));
            } else {
                ...
                if (prod.CancellaOk === false) {
                    ...
                    if (prodotto !== null && prodotto !== undefined && prodotto !== "") {
                        ...
                        if (parseInt(prodotto.Flag_Importato) === 1) {
                            kendo.alert(Traduzione(menuBSAnagraficaResx, "ProdottoImportatoNonCancellabile", "Il Prodotto è stato importato non è possibile cancellarlo"));
                            ...
                            msgTextDialog = Traduzione(menuBSAnagraficaResx, "ConfermaCancellazioneProdotto", "Si desidera davvero eliminare il prodotto aziendale dall'archivio anagrafico?");

                            if (mess !== "") {
                                msgTextDialog += "<br/><strong>" + Traduzione(menuBSAnagraficaResx, "ConfermaCancellazioneRiferimentiProdotto", "Verrano eliminati anche:") + "<br/>" + mess + "</strong>";
                            }
                        }
                    }
                    else {
                        kendo.alert(Traduzione(menuBSAnagraficaResx, "ErroreCancellazioneProdotto", "Errore non è possibile completare la cancellazione del prodotto"));
                        mostra_dialog_cancella = false;
                    }
                }
            }
            ...
            mostra_dialog_cancella = true;
            msgTextDialog = Traduzione(menuBSAnagraficaResx, "ConfermaCancellazioneStoricoDati", "Si desidera davvero eliminare lo 'Storico Prezzi' , i 'Dati Contabilità' e i 'Dati Vendita Dettaglio' associati al prodotto aziendale?");
        }
        ...
        if (mostra_dialog_cancella) {
            ...
            $("#id_dialog_cancella_prodotto").kendoDialog({
                title: Traduzione(menuBSAnagraficaResx, "ConfermaCancellazione", "Conferma Cancellazione"),
                closable: true,
                modal: {
                    preventScroll: true
                },
                ...
                actions: [{
                    text: Traduzione(menuBSAnagraficaResx, "No", "No"),
                    primary: true
                },
                {
                    text: Traduzione(menuBSAnagraficaResx, "Si", "Sì"),
                    action: function (e) {
        ...
        kendo.alert(Traduzione(menuBSAnagraficaResx, "NonSiDisponePermessiCancellareProdotto", "Non si dispone dei permessi per cancellare il prodotto."));
    }
...
function PermessoModificaProdotto(piva, sa_cod) {
    return new Promise((resolve, reject) => {
        ...
        ajaxAgronica("./MenuBs_AnagraficaProdotti.aspx/PermessoModificaProdotto",
            parametri,
            function (risposta) {
...
function Prodotto_Edit_UC_ddl_prodotto_UC_categ_prod_change(e) {
    ...
    if ($("#ddl_prodotto_UC_categ_prod").attr("last_selected_categ_prod") !== undefined &&
        ...
        var msgText = Traduzione(menuBSAnagraficaResx, "SiDesideraCambiareCategoriaProdotto_", "Si desidera davvero cambiare la Categoria Prodotto? Verranno persi i valori inseriti per la Categoria: ") +
        ...
        $("#id_dialog_change_categ_prod").kendoDialog({
            title: Traduzione(menuBSAnagraficaResx, "ConfermaCambioCategoriaProdotto", "Conferma Cambio Categoria Prodotto"),
            closable: false,
            modal: {
                preventScroll: true
            },
...
function Prodotto_Edit_UC_ddl_prodotto_UC_Regolamento_Load() {
    let Regolamenti = [{ Reg_DES: Traduzione(menuBSAnagraficaResx, "RegNessuno", "Convenzionale (Reg. Nessuno)"), Reg_COD: 1 },
        { Reg_DES: Traduzione(menuBSAnagraficaResx, "RegCE834/07", "Biologico (Reg.CE 834/07 (Ex.Reg.CE 2092/91))"), Reg_COD: 4 }
    ];
...
function Ddl_prodotto_UC_mat_prima_priorita_cdg_Read(options) {
    options.success([
        { Val: 0, Des: Traduzione(menuBSAnagraficaResx, "ProdottoPrincipale", "Prodotto principale") },
        { Val: 1, Des: Traduzione(menuBSAnagraficaResx, "ProdottoSecondario", "Prodotto secondario") }
    ]);
}
        ...
        if (is_alias === false) {
            ...
            if (grid_Alias !== undefined && grid_Alias !== null && grid_Alias !== "") {
                ...
                if (ds_grid_Alias.length > 0) {
                    ...
                    kendo.alert(Traduzione(menuBSAnagraficaResx, "ImpossibileCambiareVisibilitàProdottoPerAlias", "Non è possibile cambiare la Visibilità perché ci sono delle Descrizioni Alternative associate al Prodotto."));
            ...
            if (Prodotto_Edit_UC_Alias_Associato_A_Materia_Prima === true) {
                ...
                kendo.alert(Traduzione(menuBSAnagraficaResx, "ImpossibileCambiareVisibilitàAlias", "Non è possibile cambiare la Visibilità perché il Prodotto per Descrizioni Alternative è già stato associato a dei Prodotti."));
...
function PopolaGrigliaStoricoPrezzi(IDControllo, Disabilita_Griglia) {
    ...
    var colonneKendoGrid = [
        {
            field: "Validita_Inizio", title: Traduzione(menuBSAnagraficaResx, "ValiditàInizio", "Validità Inizio"), format: "{0:dd/MM/yyyy}"
        },
        {
            field: "Validita_Fine", title: Traduzione(menuBSAnagraficaResx, "ValiditàFine", "Validità Fine"), format: "{0:dd/MM/yyyy}"
        },
        {
            field: "Udm_Des", title: Traduzione(menuBSAnagraficaResx, "UnitàDiMisura", "Unità di Misura"), editor: unita_misura_DropDownEditor
        },
        {
            field: "Prezzo_Unitario", title: Traduzione(menuBSAnagraficaResx, "Prezzo", "Prezzo"), editor: numberEditor2decimals, format: "{0:n2}"
        }
    ];
...
function SubmitProdottiXGrigliaStoricoPrezzi(options) {
    ...
    if (nrErr > 0) {
        if (nrErr === 1)
            MessaggioErrore_Bootstrap(Traduzione(menuBSAnagraficaResx, "EsisteRigaIncompletaNellaGrigliaStoricoPrezzi", "Esiste una riga con dati non completi nella griglia Storico Prezzi."), "DIV_Messaggi");
        else
            MessaggioErrore_Bootstrap(kendo.format(Traduzione(menuBSAnagraficaResx, "EsistonoNRigheIncompleteNellaGrigliaStoricoPrezzi", "Esistono {0} righe con dati non completi nella griglia Storico Prezzi."), nrErr), "DIV_Messaggi");
...
function PopolaGrigliaCalibri(IDControllo) {
    ...
    var colonneKendoGrid = [
        {
            field: "Cal_Des", title: Traduzione(menuBSAnagraficaResx, "Calibri", "Calibri"), filterable: { multi: true, search: true }
        }
    ];
    var parametriPerLettura = null;
...
function PopolaGrigliaIndici(IDControllo) {
    ...
    var colonneKendoGrid = [
        {
            field: "IND_MAT_DES", title: Traduzione(menuBSAnagraficaResx, "IndiciDiMaturita", "Indici di Maturità"), filterable: { multi: true, search: true }
        }
    ];
...
function PopolaGrigliaTraduzioni(IDControllo, Disabilita_Griglia) {
    ...
    var colonneKendoGrid = [
        {
            field: "Nome", title: Traduzione(menuBSAnagraficaResx, "Lingua", "Lingua"), editor: Lingua_DropDownEditor
        },
        {
            field: "Mat_Des", title: Traduzione(menuBSAnagraficaResx, "Traduzione", "Traduzione")
        },
        {
            field: "DATA_AGG", title: Traduzione(menuBSAnagraficaResx, "DataAggiunta", "Data Aggiunta"), format: "{0:dd/MM/yyyy}"
        }
    ];
...
function SubmitProdottiXTraduzioni(options) {
    ...
    if (nrErr > 0) {
        if (nrErr == 1)
            MessaggioErrore_Bootstrap(Traduzione(menuBSAnagraficaResx, "EsisteRigaIncompletaNellaGrigliaTraduzioni", "Esiste una riga con dati non completi nella griglia Traduzioni."), "DIV_Messaggi");
        else
            MessaggioErrore_Bootstrap(kendo.format(Traduzione(menuBSAnagraficaResx, "EsistonoNRigheIncompleteNellaGrigliaTraduzioni", "Esistono {0} righe con dati non completi nella griglia Traduzioni."), nrErr), "DIV_Messaggi");
...
function Valida_Salvataggio(nuovo, esci) {
        ...
        if ($(xDatiImmagineCaricata).val() !== "") {
            ...
            if (rilevatoVirus !== "") {
                ...
                if (TipidiImmagineCaricabili.includes(datiImmagine.Estensione.toLowerCase()) === false) {
                    let Msg_Errore = Traduzione(menuBSAnagraficaResx, "EstensioneImmagineNonSupportata", "Estensione dell'Immagine caricata non supportata.") + "<br> " +
                        Traduzione(menuBSAnagraficaResx, "EstensioniSupportate", "Le estensioni supportate sono:") + " <strong>" + TipidiImmagineCaricabili.join(",") + "</strong>.";
    ...
    if (!valoreRitorno.Errori) {
        ...
        if ((tipoOperazione === enum_tipoOperazione.Scrittura || tipoOperazione === enum_tipoOperazione.Duplica) && esci === false && nuovo === false) {
            ...
            $.ajax({
                type: 'POST',
                url: '../MenuAnagrafica/MenuBs_AnagraficaProdotti.aspx/EditProdotto',
                data: param,
                contentType: 'application/json; charset=utf-8',
                cache: false,
                dataType: 'json', async: true,
...
function prodotto_Edit_UC_ValidaxSubmit(nuovo, esci) {
    ...
    if (tipoOperazione === enum_tipoOperazione.Scrittura && impGruppoMerceControllo == true) {
        ...
        if (kddlGruppoMerce === undefined || (kddlGruppoMerce !== undefined && kddlGruppoMerce.dataSource.data().length > 1 && parseInt(kddlGruppoMerce.value()) === 0)) {
            ...
            content.push(Traduzione(menuBSAnagraficaResx, "NonInseritoGruppoMerce", "Non è stato inserito il gruppo merce"));
    ...
    switch (elem_cod) {
        ...
        case categorieProdotti.BENI_CONFEZIONAMENTO_VEGETALI:
            ...
            if (tabStripAperti.includes("a_tab_prodotto_UC_configurazione") &&
                ...
                if ((Mostra_chkconfezione === true || Impostazione_utenteTabConfigurazione.Mostra_ChkContenitore === true ||
                    ...
                    mostraDialogConferma = true;
                    content.push(Traduzione(menuBSAnagraficaResx, "ImpostazionePesiNecessariaPerDettagliConfezionamento", "Se non viene selezionato 'Impostazione Pesi', perderai i dati di 'Dettagli Beni Di Confezionamento'"));
                    listaAzioniConferma.push(function (e) {
        ...
        case categorieProdotti.TRASFORMATI_VEGETALI:
            ...
            if (elem_cod === categorieProdotti.TRASFORMATI_VEGETALI && Modulo_FF === true && Is_OMNI === false &&
                ...
                mostraDialogConferma = true;
                content.push(Traduzione(menuBSAnagraficaResx, "NonInseritoProdottoBase", "Non è stato scelto il Prodotto Base di Riferimento per il Trasformato Vegetale"));
                //listaAzioniConferma.push(function (e) {});
            }
            break;
    }
    ...
    if (!mostraDialogConferma) {
        ...
        let strContent = "";
        if (content.length === 1) {
            strContent = Traduzione(menuBSAnagraficaResx, "RilevatoSingoloMessaggio", "È stato rilevato il seguente messaggio");
        }
        else {
            strContent = Traduzione(menuBSAnagraficaResx, "RilevatiMultipliMessaggi", "Sono stati rilevati i seguenti messaggi");
        }

        strContent += "<br/><br/>- " + content.join("<br/>-") + "<br/><br/>" + Traduzione(menuBSAnagraficaResx, "ProcedereComunqueSalvataggio", "Procedere comunque al salvataggio?");

        $("#" + id_dialog.id).kendoDialog({
            title: Traduzione(menuBSAnagraficaResx, "ConfermaSalvataggio", "Conferma Salvataggio"),
            closable: false,
            modal: {
                preventScroll: true
            },
            content: strContent,
            actions: [{
                text: Traduzione(menuBSAnagraficaResx, "Annulla", "Annulla"),
                primary: true,
            },
            {
                text: Traduzione(menuBSAnagraficaResx, "Salva", "Salva"),
                action: function (e) {
...
function Configuratore_Maschera_MateriaPrima(Elem_Cod_CategScelta) {
    ...
    switch (Elem_Cod_CategScelta) {
        case categorieProdotti.ALTRE_RISORSE:
            ...
            $('label[for="chk_prodotto_UC_Udm_Cod_Extra"]').html(Traduzione(menuBSAnagraficaResx, "ProdottoConfezionato", "Prodotto Confezionato"));
            break;
        case categorieProdotti.MATERIE_PRIME_VEGETALI:
            $("#switch_prodotto_UC_componi_descrizione").hide();
            $("#cb_prodotto_UC_componi_descrizione").data("kendoSwitch").enable(false);
        ...
        case categorieProdotti.MANGIMI:
            ...
            if (Elem_Cod_CategScelta === categorieProdotti.SEMILAVORATI_PRODUZIONE_ANIMALE ||
                ...
                $("#lbl_prodotto_UC_specie_anim").html(Traduzione(menuBSAnagraficaResx, "SpecieAnimale", "Specie Animale") + " *:");

                if (Elem_Cod_CategScelta === categorieProdotti.SEMILAVORATI_PRODUZIONE_ANIMALE) {
                    $("#lbl_prodotto_UC_indi_produt").html("");
                    $("#lbl_prodotto_UC_indi_produt").html(Traduzione(menuBSAnagraficaResx, "IndirizzoProduttivo", "Indirizzo Produttivo") + " *:");
                    $("#lbl_prodotto_UC_razza_anim").html("");
                    $("#lbl_prodotto_UC_razza_anim").html(Traduzione(menuBSAnagraficaResx, "Razza", "Razza") + " *:");
                }
            ...
            if (categorieProdotti.BENI_CONFEZIONAMENTO_ANIMALI === Elem_Cod_CategScelta ||
                ...
                if (categorieProdotti.BENI_CONFEZIONAMENTO_ANIMALI === Elem_Cod_CategScelta) {
                    $(".bene_conf").show();
                    $('label[for="chk_prodotto_UC_Udm_Cod_Extra"]').html(Traduzione(menuBSAnagraficaResx, "ImpostazionePesi", "Impostazione Pesi"));
        ...
        case categorieProdotti.SEMENTI:
            ...
            $("#lbl_prodotto_UC_sementi_materiale").html(Traduzione(menuBSAnagraficaResx, "SementeMaterialeVivaistico", "Sementi e materiale vivaistico") + " *:");

            $("#lbl_prodotto_UC_specie_veg").html("");
            $("#lbl_prodotto_UC_specie_veg").html(Traduzione(menuBSAnagraficaResx, "SpecieVegetale", "Specie Vegetale") + " *:");

            $("#lbl_prodotto_UC_varieta").html("");
            $("#lbl_prodotto_UC_varieta").html(Traduzione(menuBSAnagraficaResx, "VarietàColturale", "Varietà Colturale") + " *:");
            //############################################
            ...
            if (paramQual_FF_filtrospevar[ipar].Tipo === 3 ||
                ...
                if (parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 5 || parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 8) {
                    ...
                    if (parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 5) {
                        descr_label = Traduzione(menuBSAnagraficaResx, "NumeroDiConfezioniPerContenitore", "Numero di confezioni per contenitore");
                    } else if (parseInt(paramQual_FF_filtrospevar[ipar].Tabella_ID) === 8) {
                        descr_label = Traduzione(menuBSAnagraficaResx, "NumeroDiContenitoriPerImballo", "Numero di contenitori per imballo");
                    }
    ...
    if (switch_descrizione.check() === true) {
        if ($("#txt_prodotto_UC_Descrizione").val() !== "" && $("#txt_prodotto_UC_Descrizione").val() !== " ") {
            ...
            $("#id_dialog_genera_descr").kendoDialog({
                title: Traduzione(menuBSAnagraficaResx, "ConfermaGenerazioneDescrizione", "Conferma Generazione Descrizione"),
                closable: false,
                modal: {
                    preventScroll: true
                },
                content: Traduzione(menuBSAnagraficaResx, "SiDesideraDavveroComporreDescrizione", "Si desidera davvero comporre in automatico la Descrizione del prodotto in base ai dati compilati?</br>( Verrà sovrascritta la Descrizione inserita )"),
                actions: [{
                    text: Traduzione(menuBSAnagraficaResx, "No", "No"),
                    action: function (e) {
...
function Prodotto_Importato(Flag_Importato) {
    ...
    $("#lbl_prodotto_UC_cod_prod").html(Traduzione(menuBSAnagraficaResx, "CodiceProdotto", "Codice Prodotto") + " *:");
        ...
        //abilito textbox codice esterno e disabilito codice articolo che viene generato
            ...
            if (elem_cod === categorieProdotti.TRASFORMATI_VEGETALI) {
                ...
                if (risp === true) {
                    ...
                    $("#lbl_prodotto_UC_cod_est").html(Traduzione(menuBSAnagraficaResx, "CodiceEsterno", "Codice Esterno") + " *:");
                }
            }
    ...
    $("#prodotto_UC_imageEditor").kendoImageEditor({
        ...
        imageLoaded: function (e) {
            ...
            if ($(xDatiImmagineCaricata).val() !== "") {
                ...
                $("#prodotto_UC_lblImage").text(Traduzione(menuBSAnagraficaResx, "Immagine", "Immagine") + " '" + datiImmagine.Nome + "' :");
...
$("#btn_prodotto_UC_rimuovi_immagine").on("click", function () {
    ...
    $("#prodotto_UC_lblImage").text(Traduzione(menuBSAnagraficaResx, "Immagine", "Immagine") + ":");
...
function PopolaGrigliaAlias(IDControllo, Disabilita_Griglia) {
    ...
    var colonneKendoGrid = [
        {
            field: "Mat_Des_Alias", title: Traduzione(menuBSAnagraficaResx, "Prodotto", "Prodotto"), editor: Alias_DropDownEditor, filterable: { multi: true, search: true }
        },
        {
            field: "Filtro_Contatti_Des_String", title: Traduzione(menuBSAnagraficaResx, "Contatti", "Contatti"), editor: Contatti_MultiSelectEditor, filterable: { multi: true, search: true }
        }
    ];
...
function SubmitProdottiXAlias(options) {
    ...
    if (nrErr > 0) {
        if (nrErr === 1)
            MessaggioErrore_Bootstrap(Traduzione(menuBSAnagraficaResx, "EsisteRigaIncompletaNellaGrigliaAlias", "Esiste una riga con dati non completi nella griglia delle Descrizioni Alternative."), "DIV_Messaggi");
        else
            MessaggioErrore_Bootstrap(kendo.format(Traduzione(menuBSAnagraficaResx, "EsistonoNRigheIncompleteNellaGrigliaAlias", "Esistono {0} righe con dati non completi nella griglia delle Descrizioni Alternative."), nrErr), "DIV_Messaggi");
