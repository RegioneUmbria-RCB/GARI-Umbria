...
Public Class AgendaBootstrap
    ...
    Public Property flag_pag_DocContabile As Boolean
        Get
            Return agroMasterPage.flag_pag_DocContabile
        End Get
        Set(value As Boolean)
            agroMasterPage.flag_pag_DocContabile = value
        End Set
    End Property
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        agroMasterPage.SitoOspite = Enum_SiteRedirector.Sito_AgronicaAgenda_2010
        ...
        If objParametri_Server.FinestraTemporaleInizio = "01/01/1900" And objParametri_Server.FinestraTemporaleFine = "31/12/2100" Then
            lblFinestraTemporale.Text = Resources.AgronicaAgenda_2010.VisualizzazioneIllimitata
        Else
            lblFinestraTemporale.Text = Resources.AgronicaAgenda_2010.VisualizzazioneLimitataTraIl &
        ...
        agroMasterPage.SetMenuBS2017(0, objParametriAgenda.Piva, False)
        ...
        ConfiguraAgroMeteo(objParametriAgenda.Piva, objParametri_Server)
        MostraAssistenza(objParametri_Super_Server, objParametri_Server)

    End Sub

    ' Imposta città per previsioni meteo
    Private Sub ConfiguraAgroMeteo(Piva As String, objParametri_Server As AgronicaCoreDataProvider.AgronicaCoreParametri)

        ' se non è selezionata l'azienda uso l'azienda superuser
        Dim PivaAzienda = IIf(Piva <> "", Piva, objParametri_Server.PivaSuperUser)

        If PivaAzienda <> "" Then

            If Not flag_MenuBS_2017 Then

                Dim CentriAziendaliLeggi As New AgronicaCoreAnagrafeDAL.CentriAziendali_Read
                Dim dtCentriAziendali As DataTable = CentriAziendaliLeggi.CentroAziendaliLatLongDescrizione(Piva, 0, objParametri_Server)

                If dtCentriAziendali.Rows.Count > 0 Then
                    masterAgroMeteoLatitudine = CDbl(dtCentriAziendali(0)("lat")).ToString.Replace(",", ".")
                    masterAgroMeteoLongitudine = CDbl(dtCentriAziendali(0)("long")).ToString.Replace(",", ".")
                End If

            End If

            If masterAgroMeteoLatitudine = "0" AndAlso masterAgroMeteoLongitudine = "0" Then
                ...
                If dtIndirizzo.Rows.Count > 0 Then
                    ...
                    If (citta = "" OrElse citta = "Non Definita") AndAlso frazione <> "" Then
                        masterAgroMeteoDescrizione = frazione
                    Else
                        masterAgroMeteoDescrizione = citta
                    End If

                    masterAgroMeteoDescrizione &= "," + stato
                End If
    ...
    Public Function TrovaRedirectCorretto(ByVal online As Boolean,
                                          ByVal paginaRichiesta As enum_PagineGiasOnline,
                                          ByVal objParametriAgenda As ParametriAgenda,
                                          Optional ByVal usePaginaRichiestaAgenda As Boolean = False,
                                          Optional ByVal Qs_Visibilita As Integer = 0) As String
            ...
            If paginaRichiesta = enum_PagineGiasOnline.MenuAnagrafica Then
                ...
                Else
                    ...
                    If Qs_Visibilita <> 0 Then
                        TargetRedirect &= "?visibilita=" & CStr(Qs_Visibilita)
                    End If
                End If
        ...
        If online = False Then
            If usePaginaRichiestaAgenda = False Then
                ...
                Select Case paginaRichiesta
                    Case enum_PagineGiasOnline.MenuAnagrafica
                        Dim TargetRedirect = "../MenuAnagrafica/Menubs_anagrafica.aspx"
                        If Qs_Visibilita <> 0 Then
                            TargetRedirect &= "?visibilita=" & CStr(Qs_Visibilita)
                        End If
                        Return TargetRedirect

                    Case enum_PagineGiasOnline.MenuAnagraficaProdotti
                        Dim TargetRedirect = "../MenuAnagrafica/MenuBS_AnagraficaProdotti.aspx"
                        Return TargetRedirect
