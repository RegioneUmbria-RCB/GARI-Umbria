...
function CaricaGrigliaMagCont(deferred) {

    //Leggo da db solo se non ho mai letto
    if ($("#divKendoMagCont").html() === '') {
        ...
        KendoMagCont_leggi(undefined, data_inizio, data_fine, deferred);
    }

}

function KendoMagCont_inizializza(divKendoMagCont) {

    var funzioniCRUD = {
        funzioneRead: kMagContReadValorizzazione_rows,
        checkBoxFunction: KendoMagCont_checked
    };

    var idModel = "id_agenda";

    var campiKendoModel = kMagContReadValorizzazione_mod();
    var colonneKendoGrid = kMagContReadValorizzazione_col();

    var pulsanteCopiaMagCont = "<div class='btn btn-warning btnCopia' style='display:block;width:70px;border:0px;margin-bottom:0px;' onclick=duplicaElemento(this.closest('tr'),this.closest('.k-grid'))>Copia</div>";
    if (Request_QueryString("gis") === "true") {
        pulsanteCopiaMagCont = "";
    }
    ...
    var KendoMagCont = creaKendoGrid(divKendoMagCont, // rappresenta l'ID del div a cui si associa la griglia
    ...
    var griglia = $('#' + divKendoMagCont).data("kendoGrid");
    griglia.autoFitColumn("Data2");

}

function kMagContReadValorizzazione_rows(options) {

    var data = $('#hdKendoMagCont_Valorizzazione').val();
    jSonParsed_Kendo = JSON.parse(data);

    options.success(jSonParsed_Kendo.kendo_rows);
}

function kMagContReadValorizzazione_col() {
...
function kMagContReadValorizzazione_mod() {
...
function KendoMagCont_checked(e) {

    let checked = this.checked;
    let row = $(this).parents("tr");
    let grid = $("#divKendoMagCont").data("kendoGrid");
