...
function CaricaGrigliaColturali(deferred) {

    //Leggo da db solo se non ho mai letto
    if ($("#divKendoColturali").html() === '') {
        ...
        KendoColturali_leggi(undefined, data_inizio, data_fine, sa_cod, veg_cod, deferred);
    }

}

function KendoColturali_inizializza(divKendoColturali) {

    var funzioniCRUD = {
        funzioneRead: kColturaliReadValorizzazione_rows,
        checkBoxFunction: KendoColturali_checked
    };

    var idModel = "id_agenda";

    var campiKendoModel = kColturaliReadValorizzazione_mod();
    var colonneKendoGrid = kColturaliReadValorizzazione_col();

    var pulsanteCopiaColturali = "<div class='btn btn-warning btnCopia' style='display:block;width:70px;border:0px;margin-bottom:0px;' onclick=duplicaElemento(this.closest('tr'),this.closest('.k-grid'))>Copia</div>";
    if (Request_QueryString("gis") === "true") {
        pulsanteCopiaColturali = "";
    }
    ...
    var KendoColturali = creaKendoGrid(divKendoColturali, // rappresenta l'ID del div a cui si associa la griglia
    ...
    var griglia = $('#' + divKendoColturali).data("kendoGrid");
    griglia.autoFitColumn("Data2");

}

function kColturaliReadValorizzazione_rows(options) {

    var data = $('#hdKendoColturali_Valorizzazione').val();
    jSonParsed_Kendo = JSON.parse(data);

    options.success(jSonParsed_Kendo.kendo_rows);
}

function kColturaliReadValorizzazione_col() {
...
function kColturaliReadValorizzazione_mod() {
...
function KendoColturali_checked(e) {

    let checked = this.checked;
    let row = $(this).parents("tr");
    let grid = $("#divKendoColturali").data("kendoGrid");
