$(document).ready(function () {
    let mainContainer = document.getElementById("widget");
    ...
    if (weatherSummaryAllowed) {
        createTabStrip(mainContainer);

        $("#widget").RiepilogoMeteo({
            url_meteoWS: "../../MeteoWS.aspx",
            piva: piva,
            summary: false,
            fun_callbackShowResults: ShowRiepilogo
        });
    }
...
function ShowRiepilogo(stations) {

    if (stations == undefined || stations == null || stations.lenght == 0 || !weatherSummaryAllowed) {
        $(".window-loader").each(function (i, e) { e.remove(); });
        return;
    }
