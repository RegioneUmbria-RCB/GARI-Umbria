...
function MostraIndicatori(div, Centri) {
        ...
        if (RitardaCalcolo) {
            ...
            $("#" + idIndicCentro).on(ev, function () {
                $("#" + idIndicCentro + " > .indic-grid > .indic-elem ").each(function (idx, elem) {
                    let plugin = $(elem).data("WBResult_APP_V2");
                    if (plugin !== undefined) {
                        plugin.run();
                    }
                });
    ...
    if (stazioniXcentro.length > 0) {
        ...
        if (typeof window.Worker === "function") {
            ...
            Utility_Worker = new Worker("../DSS_Irrigazione/DSS_Utility_Worker.js");
...
function creaWidget(campo, indic, parentDiv, runFlag, allowPersistence) {
    ...
    $(indWrapper).WBResult_APP_V2(header, coltura, arrIndic, allowPersistence);

    if (runFlag)
        $(indWrapper).data("WBResult_APP_V2").run();
}
