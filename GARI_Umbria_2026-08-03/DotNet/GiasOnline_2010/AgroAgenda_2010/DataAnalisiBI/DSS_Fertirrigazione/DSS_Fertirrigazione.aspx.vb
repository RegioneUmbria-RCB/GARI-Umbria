...
Public Class DSS_Fertirrigazione
    ...
    Public Shared Function MessaggioANBI() As RispostaStandard
        ...
        MsgFull = "Il sistema GIAS integra il <strong>Servizio IRRIFRAME</strong>. Irriframe è un progetto <a href='http://www.anbi.it' target='_blank'>ANBI</a> "
        ...
        MsgFull &= "</br>"
        MsgFull &= "Si ricorda che il <strong>Servizio IRRIFRAME è fruibile gratuitamente e indipendentemente da GIAS sul sito <a href='http://www.irriframe.it' target='_blank'>irriframe</a>.</strong>"
    ...
    Public Class Indicatore
        ...
        Public Sub New(row As DataRow)
            ...
            If Not IsDBNull(row("Cop_Cod")) Then
                Dim noProtettaCop = New HashSet(Of Integer)({0, 1, 3, 4, 5, 15})
                Dim copCod = row("Cop_Cod")
                If (Not noProtettaCop.Contains(copCod)) Then
                    If IsDBNull(row("Cop_Cod_Irriframe")) Then
                        Errore = "Mappatura copertura mancante"
                    Else
                        ColturaProtetta = True
                        Id_GreenHouseType = row("Cop_Cod_Irriframe")
                    End If
                End If
                End If
            ...
            FertiRecipe = Nothing
            If Not String.IsNullOrEmpty(row("FertiRecipe")) Then
                Dim fertiRecipeJSON = JObject.Parse(row("FertiRecipe"))
                FertiRecipe = New _FertiRecipe With {
                    .N_Ammesso = fertiRecipeJSON("N_Ammesso"),
                    .P_Ammesso = fertiRecipeJSON("P_Ammesso"),
                    .K_Ammesso = fertiRecipeJSON("K_Ammesso")
                }
            End If

            FertIrriGList = Nothing
            If Not String.IsNullOrEmpty(row("Fertilizzazioni")) Then
                Dim fertiArr = JArray.Parse(row("Fertilizzazioni").ToString)
                FertIrriGList = New List(Of _FertiIrriG)
                For Each fertiObj As JObject In fertiArr
                    FertIrriGList.Add(New _FertiIrriG With {
                        .Data = CDate(fertiObj("Data")),
                        .TitoloN = fertiObj("TitoloN"),
                        .TitoloP = fertiObj("TitoloP"),
                        .TitoloK = fertiObj("TitoloK"),
                        .DoseKg = fertiObj("DoseKg"),
                        .NomeCommercialeDescri = fertiObj("NomeCommercialeDescri")
                    })
                Next
            End If
    ...
    Public Shared Function CaricaIndicatori() As RispostaStandard
        ...
        Try
            ...
            Dim DT As DataTable = objRead.Leggi_x_DSS_Irrigazione(objParametriAgenda.Piva, json_txcod, json_tx_irricod, json_ff_start, objParametri_Server, True)
    ...
    Public Shared Function IrriframeAllowPersitence() As RispostaStandard
        ...
        Try
            ...
            r.RispostaOK = True
            r.RispostaStringa = New JObject(New JProperty("AllowPersistence", if_api.Persistent())).ToString()
    ...
    Public Shared Function SpecieCaricaCombo() As RispostaStandard
        ...
        Try
            ...
            r.RispostaOK = True
            Dim elementiComboSpecie As String = AgronicaCoreUtility.CaricaListControl.itemsCollection_AsJsonArray(cmbSpecie.ddl_Specie.Items, "veg_cod", "veg_des")
            r.RispostaStringa = elementiComboSpecie
