...
$(document).ready(function () {
    ...
    if (typeof window.Worker === "function") {
        ...
        DSS_Worker = new Worker("DSS_Fertirrigazione_Worker.js");
...
function ElaboraIndicatori(indicatori) {

    MostraIndicatori("DSS_Irrigazione_Indicatori", indicatori);

    let vegArray = [];

    let seen_obj = {};
    $.each(indicatori, function (idx, indic) {
        if (!seen_obj.hasOwnProperty("S_" + indic.Veg_Cod)) {
            seen_obj["S_" + indic.Veg_Cod] = 1;
            vegArray.push(indic.Veg_Cod);
        }
    });
