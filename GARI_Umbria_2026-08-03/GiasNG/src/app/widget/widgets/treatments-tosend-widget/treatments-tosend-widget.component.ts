...
import { TreatmentsToSendWidgetGridConfig } from './treatments-tosend-grid-widget-config.service';
...
@Component({
  standalone: false,
  selector: 'app-treatments-tosend-widget',
  templateUrl: './treatments-tosend-widget.component.html',
  styleUrls: ['./treatments-tosend-widget.component.css'],
  providers: [
      ...generateGridProviders(TreatmentsToSendWidgetGridConfig, TreatmentsToSendWidgetComponent),
      GiasDropDownTemplateService,
      GiasMultiSelectTemplateService
    ]
})
export class TreatmentsToSendWidgetComponent implements OnInit, OnDestroy {
  ...
  constructor(
    @Inject(GRID_HTTP_TOKEN) private gridAlertService: TreatmentsToSendWidgetGridConfig,
  ...
  ngOnInit(): void {
    ...
    this.loadStalleDDL(this.piva, 0)
      .subscribe((stalleResult) => {
        ...
        let cookie = this.gridAlertService.getTreatmentsToSendCookie();
        if (cookie) {
  ...
  ngOnDestroy(): void {
    ...
    this.gridAlertService.setTreatmentsToSendCookie(params);
  ...
  redirectToTreatmentsToSend() {
    this.gridAlertService.redirectToTreatmentsToSend();
  }
}
