...
import { ContoEconomicoService } from "app/valutazioni/valutazioni/service/conto-economico.service";
import { ValutazioniService } from "app/valutazioni/valutazioni/service/valutazioni.service";
import { ContoEconomicoModel, ContoEconomicoKendoServerResult } from "app/valutazioni/valutazioni/valutazioni.models";
import { takeUntil, take, Observable, map, switchMap, of } from "rxjs";
import { ObjParametriAgenda } from 'gias-ui-kit';


@Injectable()
export class ContoEconomicoHttpService extends AbstractGridConfigService<any> {
    gridId = 'ContoEconomicoHttpService';
    ...
    constructor(injector: Injector,
        ...
        this.model = {
            ...
            Dettaglio_Specifico: new ModelEntry(CELL_TYPES.OBJECT, false),
        ...
        this.masterdetailSettings = new MasterDetailSettings(true, {
            ...
            showDetailTemplate: (dataitem: any, rowIndex: number) => dataitem.Dettaglio_Specifico != '[]'
        });
    ...
    read(): Observable<ContoEconomicoKendoServerResult> {
        let objParametriAgenda = this.objParametriService.getObjParamValue();
        let testata_lettura = this.valutazioneService.getLeggiTestataValue();

        return this.contoEconomicoService.leggiContoEconomico(objParametriAgenda.Piva, testata_lettura.idTestata).pipe(
            map((data: any[]) => this.transform(data))
        );
    }

    private transform(data: any[]): ContoEconomicoKendoServerResult {
            ...
            case HttpAction.UPDATE:
                ...
                return this.valutazioneService.scriviDettaglioGriglia(JSON.stringify(items)).pipe(
                    switchMap((resp) => {
                        ...
                        if (resp.RispostaOK) {
                            this.giasMessageService.successMessage(this.translocoService.translate('ContoEconomicoSalvatoCorettamente'));
                        }
                        return of([]);
                    })
                );
