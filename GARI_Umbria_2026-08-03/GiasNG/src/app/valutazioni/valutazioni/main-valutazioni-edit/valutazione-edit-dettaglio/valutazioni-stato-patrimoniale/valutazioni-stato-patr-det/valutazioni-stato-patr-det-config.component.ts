...
import { DettaglioSpecificoServerResult, DettaglioSpecificoModel } from "app/valutazioni/valutazioni/valutazioni.models";
import { takeUntil, take, Observable, of, switchMap, Subscription } from "rxjs";
import { ObjParametriAgenda } from 'gias-ui-kit';

@Injectable()
export class StatoPatrimonialeDettaglioHttpService extends AbstractGridConfigService<any> {
    gridId = 'StatoPatrimonialeDettaglioHttpService';
    ...
    constructor(injector: Injector,
        ...
        this.masterdetailSettings = new MasterDetailSettings(true, { flag_grid_detail: true, flag_grid_master: false });
        ...
        switch (actionType) {
            ...
            case HttpAction.UPDATE:
                ...
                return this.valutazioniService.scriviDettaglioGrigliaFiglia(JSON.stringify(items)).pipe(
                    switchMap((resp) => {
                        if (resp.RispostaOK) {
                            this.giasMessageService.successMessage(this.translocoService.translate('StatoPatrimonialeFiglioSalvatoCorettamente'));
