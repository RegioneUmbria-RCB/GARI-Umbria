...
export class AjaxAgronicaAPIService implements IGiasApiService {
  private pako;
  ...
  ajaxAPIPost<InType,OutType>(url: string,
    ...
    let link = ""
    if (url[0] == "/"){
      link = this.masterService.link_API + url;
    } else {
      link = this.masterService.link_API + "/" + url;
    }
  ...
  ajaxAPIPostFormData<OutType>(
    url: string, parametri: FormData,
    setLoading: boolean = false, compressione: boolean = true,
    showErroriGestiti: boolean = true, showErroriNonGestiti: boolean = true
    ...
    return this.ajaxPostFormData(link, parametri, setLoading, compressione,showErroriGestiti, showErroriNonGestiti) as Observable<rispostaStandard<OutType>>;
  }
  ...
  private ajaxPostFormData<OutType>(
    url: string, parametri: FormData,
    setLoading: boolean = false, compressione: boolean = true,
    showErroriGestiti: boolean = true, showErroriNonGestiti: boolean = true
  ): Observable<rispostaStandard<OutType>> {
    ...
    // A quanto pare non vuole le opzioni aggiuntive
    const risposta = this.http.post(url, parametri)//, httpOptions)
      .pipe(
        catchError((err) => {
          return this.errorHandling(err);
        }),
        filter((r) => {
          if (<string>r == "401") {
            if (setLoading) {
              this.masterService.set_isLoading({isLoading: false, message: 'Caricamento in corso'});
            }
            return false;
          }
          return true;
        }),
        map((r) => (<rispostaStandard<OutType>>r)),
        ...
        map((r: rispostaStandard<OutType>) => {
          if (r.RispostaOK === true) {
            let obj = r.RispostaStringa;
            if (typeof r.RispostaStringa == 'string' && r.RispostaStringa != "" ) { // && isJSON(r.RispostaStringa)
              try {
  ...
  private ajaxGet<InType, OutType>(url,
    ...
    const risposta = this.http.get<any>(url, { params: queryParams, headers: headers, withCredentials: true })
      .pipe(
        ...
        map((r) => {
          return (<rispostaStandard<InType>>r);
        }),
        map((val) => {
          return this.decomprimi(val);
        }),
