...
// ------------------- LA NOSTRA APPKEY -------------------
// --------------------------------------------------------
var OWM_API_KEY = '185477576fdf6d45a5da2ed59a801fb3';
...
// --------------------------------------------------------
// ---------------- COORDINATE DI INTERESSE ---------------
// --------------------------------------------------------
// di default punta Cesena
var latitudineAgroWS = 44.12;
var longitudineAgroWS = 12.26;
// --------------------------------------------------------
// -------------------- PERCORSO ICONE --------------------
// --------------------------------------------------------
var percorsoAssolutoIcone = "";
...
// --------------------------------------------------------
// -------------- INIT DELLA MAPPA A LAYER ----------------
// --------------------------------------------------------
function initMap(latitudine, longitudine, percorso) {

    // latitudine, longitudine e percorso icone assoluto
    latitudineAgroWS = latitudine;
    longitudineAgroWS = longitudine;
    percorsoAssolutoIcone = percorsoIcone.substring(0, percorso.length - 7);
    ...
    var clouds = L.OWM.clouds({ opacity: 0.8, legendImagePath: percorsoAssolutoIcone + 'NT2.png', appId: OWM_API_KEY });
    ...
    // come linguaggio metto fisso l'inglese: è stato tradotto in italiano
    var localLang = 'en';

    var city = L.OWM.current({
        intervall: 15, imageLoadingUrl: percorsoAssolutoIcone + 'owmloading.gif', lang: localLang, minZoom: 5,
        appId: OWM_API_KEY
    });
    var station = L.OWM.current({
        type: 'station', intervall: 15, imageLoadingUrl: percorsoAssolutoIcone + 'owmloading.gif', lang: localLang,
        appId: OWM_API_KEY /* , markerFunction: myOwmMarker, popupFunction: myOwmPopup */
    });
    var windrose = L.OWM.current({
        intervall: 15, imageLoadingUrl: percorsoAssolutoIcone + 'owmloading.gif', lang: localLang, minZoom: 4,
    ...
    // geolocalizzazione e zoom della mappa: viene puntata sulla zona in cui è locata l'azienda di interesse
    var zoom = 11;
    map = L.map('mappa_interattiva', {
        center: new L.LatLng(latitudineAgroWS, longitudineAgroWS), zoom: zoom,
        layers: [standard]
    });
    map.attributionControl.setPrefix("");

    // mappa politica e mappa fisica
    var baseMaps = {
    ...
    // aggiungo il menu di scelta dei layer da visualizzare al lato destro della mappa
